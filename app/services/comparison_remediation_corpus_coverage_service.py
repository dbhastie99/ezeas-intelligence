...
from app.services.domain_retrieval_plan_service import COMPARISON_REMEDIATION_PLAN, EvidenceGroup
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


@dataclass(frozen=True)
class ComparisonRemediationGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class ComparisonRemediationCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[ComparisonRemediationGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Comparison / Remediation claims.",
        ]
    return [
...
def scan_comparison_remediation_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> ComparisonRemediationCoverageReport:
    group_reports: list[ComparisonRemediationGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in COMPARISON_REMEDIATION_PLAN.evidence_groups:
        ...
        group_reports.append(
            ComparisonRemediationGroupCoverage(
    ...
    return ComparisonRemediationCoverageReport(
        plan_id=COMPARISON_REMEDIATION_PLAN.plan_id,
        domain=COMPARISON_REMEDIATION_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(COMPARISON_REMEDIATION_PLAN.evidence_groups),
