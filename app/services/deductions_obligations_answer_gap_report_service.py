...
CORE_DEDUCTIONS_OBLIGATIONS_GROUPS = {
...
class DeductionsObligationsAnswerGapReportError(ValueError):
    pass


@dataclass(frozen=True)
class DeductionsObligationsAnswerGapFinding:
    group_key: str
    coverage_status: str
    answer_impact: str
...
@dataclass(frozen=True)
class DeductionsObligationsAnswerGapReport:
    report_type: str
    generated_at_utc: str
    overall_status: str
    group_findings: list[DeductionsObligationsAnswerGapFinding]
...
def validate_deductions_obligations_coverage_report(coverage_report: dict) -> None:
    if not isinstance(coverage_report, dict):
        raise DeductionsObligationsAnswerGapReportError("Coverage report must be a JSON object.")
    if coverage_report.get("plan_id") != "DEDUCTIONS_OBLIGATIONS":
        raise DeductionsObligationsAnswerGapReportError("Coverage report must have plan_id DEDUCTIONS_OBLIGATIONS.")
    if not isinstance(coverage_report.get("groups"), list):
        raise DeductionsObligationsAnswerGapReportError("Coverage report must include groups[].")
    if not coverage_report["groups"]:
        raise DeductionsObligationsAnswerGapReportError("Coverage report groups[] must not be empty.")

    for index, group in enumerate(coverage_report["groups"]):
        if not isinstance(group, dict):
            raise DeductionsObligationsAnswerGapReportError(f"Coverage group at index {index} must be an object.")
        group_key = group.get("group_key")
        coverage_status = group.get("coverage_status")
        if not group_key:
            raise DeductionsObligationsAnswerGapReportError(f"Coverage group at index {index} is missing group_key.")
        if coverage_status not in VALID_COVERAGE_STATUSES:
            raise DeductionsObligationsAnswerGapReportError(
...
def _finding_for_group(group: dict) -> DeductionsObligationsAnswerGapFinding:
    group_key = group["group_key"]
    status = group["coverage_status"]
    is_core = group_key in CORE_DEDUCTIONS_OBLIGATIONS_GROUPS

    if status == "STRONG":
        return DeductionsObligationsAnswerGapFinding(
            group_key=group_key,
            coverage_status=status,
            answer_impact="LOW",
            diagnostic_summary=_diagnostic_summary(group),
            recommended_action="KEEP",
            rationale="Formal-corpus coverage is strong enough for the current Deductions / Obligations answer standard.",
        )
    if status == "WEAK":
        ...
        return DeductionsObligationsAnswerGapFinding(
            ...
            rationale=(
                "Core Deductions / Obligations support exists but may need clearer answer synthesis."
                if is_core
    ...
    return DeductionsObligationsAnswerGapFinding(
        ...
        rationale=(
            "This is a core Deductions / Obligations concept and missing evidence will materially weaken answers."
            if is_core
...
def _overall_status(findings: list[DeductionsObligationsAnswerGapFinding]) -> str:
    core_findings = [finding for finding in findings if finding.group_key in CORE_DEDUCTIONS_OBLIGATIONS_GROUPS]
...
def _recommended_next_actions(
    findings: list[DeductionsObligationsAnswerGapFinding],
    overall_status: str,
) -> list[str]:
    actions: list[str] = []
    if overall_status == "GOOD":
        actions.append("Keep current Deductions / Obligations retrieval terms and answer synthesis under benchmark watch.")
    if any(f.recommended_action == "ADD_FORMAL_SOURCE_EVIDENCE_LATER" for f in findings):
        actions.append("Add formal source evidence later for missing Deductions / Obligations groups before widening answer claims.")
    if any(f.recommended_action == "IMPROVE_RETRIEVAL_TERMS" for f in findings):
        actions.append("Refine Deductions / Obligations retrieval terms for weak supporting groups before adding new corpus.")
    if any(f.recommended_action == "IMPROVE_SYNTHESIS" for f in findings):
        actions.append("Tighten Deductions / Obligations answer synthesis for weak core groups while keeping status caveats.")
    if overall_status == "INSUFFICIENT_CORPUS":
        actions.append("Treat Deductions / Obligations answers as corpus-limited until core evidence groups are covered.")
    return list(dict.fromkeys(actions))


def build_deductions_obligations_answer_gap_report(coverage_report: dict) -> DeductionsObligationsAnswerGapReport:
    validate_deductions_obligations_coverage_report(coverage_report)
    findings = [_finding_for_group(group) for group in coverage_report["groups"]]
    overall_status = _overall_status(findings)
    return DeductionsObligationsAnswerGapReport(
        report_type="DEDUCTIONS_OBLIGATIONS_ANSWER_GAP_REPORT",
