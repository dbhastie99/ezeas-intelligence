...
from app.services.domain_retrieval_plan_service import EvidenceGroup, LEAVE_ACCRUAL_PROCESSING_PLAN
...
class LeaveAccrualProcessingGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class LeaveAccrualProcessingCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[LeaveAccrualProcessingGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Leave Accrual / Processing claims.",
        ]
    return [
...
def scan_leave_accrual_processing_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> LeaveAccrualProcessingCoverageReport:
    group_reports: list[LeaveAccrualProcessingGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in LEAVE_ACCRUAL_PROCESSING_PLAN.evidence_groups:
        ...
        group_reports.append(
            LeaveAccrualProcessingGroupCoverage(
    ...
    return LeaveAccrualProcessingCoverageReport(
        plan_id=LEAVE_ACCRUAL_PROCESSING_PLAN.plan_id,
        domain=LEAVE_ACCRUAL_PROCESSING_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(LEAVE_ACCRUAL_PROCESSING_PLAN.evidence_groups),
