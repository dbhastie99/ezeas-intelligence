...
from app.services.domain_retrieval_plan_service import EvidenceGroup, LEAVE_SOURCE_MODEL_PLAN
...
class LeaveSourceModelGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class LeaveSourceModelCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[LeaveSourceModelGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Leave Source Model claims.",
        ]
    return [
...
def scan_leave_source_model_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> LeaveSourceModelCoverageReport:
    group_reports: list[LeaveSourceModelGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in LEAVE_SOURCE_MODEL_PLAN.evidence_groups:
    ...
    return LeaveSourceModelCoverageReport(
        plan_id=LEAVE_SOURCE_MODEL_PLAN.plan_id,
        domain=LEAVE_SOURCE_MODEL_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(LEAVE_SOURCE_MODEL_PLAN.evidence_groups),
