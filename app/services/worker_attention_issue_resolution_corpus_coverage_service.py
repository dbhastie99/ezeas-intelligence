...
from app.services.domain_retrieval_plan_service import WORKER_ATTENTION_ISSUE_RESOLUTION_PLAN, EvidenceGroup
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


@dataclass(frozen=True)
class WorkerAttentionIssueResolutionGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class WorkerAttentionIssueResolutionCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[WorkerAttentionIssueResolutionGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Worker Attention / Issue Resolution claims.",
        ]
    return [
...
def scan_worker_attention_issue_resolution_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> WorkerAttentionIssueResolutionCoverageReport:
    group_reports: list[WorkerAttentionIssueResolutionGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in WORKER_ATTENTION_ISSUE_RESOLUTION_PLAN.evidence_groups:
        ...
        group_reports.append(
            WorkerAttentionIssueResolutionGroupCoverage(
    ...
    return WorkerAttentionIssueResolutionCoverageReport(
        plan_id=WORKER_ATTENTION_ISSUE_RESOLUTION_PLAN.plan_id,
        domain=WORKER_ATTENTION_ISSUE_RESOLUTION_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(WORKER_ATTENTION_ISSUE_RESOLUTION_PLAN.evidence_groups),
