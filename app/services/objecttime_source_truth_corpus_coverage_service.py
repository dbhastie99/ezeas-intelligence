...
from app.services.domain_retrieval_plan_service import OBJECTTIME_SOURCE_TRUTH_PLAN, EvidenceGroup
...
class ObjectTimeSourceTruthGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class ObjectTimeSourceTruthCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[ObjectTimeSourceTruthGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening ObjectTime / Source Truth claims.",
        ]
    return [
...
def scan_objecttime_source_truth_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> ObjectTimeSourceTruthCoverageReport:
    group_reports: list[ObjectTimeSourceTruthGroupCoverage] = []
    ...
    for group in OBJECTTIME_SOURCE_TRUTH_PLAN.evidence_groups:
        ...
        group_reports.append(
            ObjectTimeSourceTruthGroupCoverage(
    ...
    return ObjectTimeSourceTruthCoverageReport(
        plan_id=OBJECTTIME_SOURCE_TRUTH_PLAN.plan_id,
        domain=OBJECTTIME_SOURCE_TRUTH_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(OBJECTTIME_SOURCE_TRUTH_PLAN.evidence_groups),
