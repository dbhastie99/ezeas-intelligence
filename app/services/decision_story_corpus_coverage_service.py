...
from app.services.domain_retrieval_plan_service import DECISION_STORY_PLAN, EvidenceGroup
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


@dataclass(frozen=True)
class DecisionStoryGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class DecisionStoryCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[DecisionStoryGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Decision Story claims.",
        ]
    return [
...
def scan_decision_story_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> DecisionStoryCoverageReport:
    group_reports: list[DecisionStoryGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in DECISION_STORY_PLAN.evidence_groups:
    ...
    return DecisionStoryCoverageReport(
        plan_id=DECISION_STORY_PLAN.plan_id,
        domain=DECISION_STORY_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(DECISION_STORY_PLAN.evidence_groups),
