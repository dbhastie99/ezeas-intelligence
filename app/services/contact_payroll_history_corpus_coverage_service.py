...
from app.services.domain_retrieval_plan_service import CONTACT_PAYROLL_HISTORY_PLAN, EvidenceGroup
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


@dataclass(frozen=True)
class ContactPayrollHistoryGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class ContactPayrollHistoryCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[ContactPayrollHistoryGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Contact Payroll History claims.",
        ]
    return [
...
def scan_contact_payroll_history_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> ContactPayrollHistoryCoverageReport:
    group_reports: list[ContactPayrollHistoryGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in CONTACT_PAYROLL_HISTORY_PLAN.evidence_groups:
        ...
        group_reports.append(
            ContactPayrollHistoryGroupCoverage(
    ...
    return ContactPayrollHistoryCoverageReport(
        plan_id=CONTACT_PAYROLL_HISTORY_PLAN.plan_id,
        domain=CONTACT_PAYROLL_HISTORY_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(CONTACT_PAYROLL_HISTORY_PLAN.evidence_groups),
