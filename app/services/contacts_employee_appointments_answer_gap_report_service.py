...
CORE_CONTACTS_EMPLOYEE_APPOINTMENTS_GROUPS = {
...
class ContactsEmployeeAppointmentsAnswerGapReportError(ValueError):
    pass


@dataclass(frozen=True)
class ContactsEmployeeAppointmentsAnswerGapFinding:
    group_key: str
    coverage_status: str
    answer_impact: str
...
@dataclass(frozen=True)
class ContactsEmployeeAppointmentsAnswerGapReport:
    report_type: str
    generated_at_utc: str
    overall_status: str
    group_findings: list[ContactsEmployeeAppointmentsAnswerGapFinding]
...
def validate_contacts_employee_appointments_coverage_report(coverage_report: dict) -> None:
    if not isinstance(coverage_report, dict):
        raise ContactsEmployeeAppointmentsAnswerGapReportError("Coverage report must be a JSON object.")
    if coverage_report.get("plan_id") != "CONTACTS_EMPLOYEE_APPOINTMENTS":
        raise ContactsEmployeeAppointmentsAnswerGapReportError(
            "Coverage report must have plan_id CONTACTS_EMPLOYEE_APPOINTMENTS."
        )
    if not isinstance(coverage_report.get("groups"), list):
        raise ContactsEmployeeAppointmentsAnswerGapReportError("Coverage report must include groups[].")
    if not coverage_report["groups"]:
        raise ContactsEmployeeAppointmentsAnswerGapReportError("Coverage report groups[] must not be empty.")
...
def _finding_for_group(group: dict) -> ContactsEmployeeAppointmentsAnswerGapFinding:
    group_key = group["group_key"]
    status = group["coverage_status"]
    is_core = group_key in CORE_CONTACTS_EMPLOYEE_APPOINTMENTS_GROUPS

    if status == "STRONG":
        return ContactsEmployeeAppointmentsAnswerGapFinding(
            group_key=group_key,
            ...
            rationale=(
                "Formal-corpus coverage is strong enough for the current Contacts / Employee Appointments "
                "answer standard."
            ),
        )
    if status == "WEAK":
        ...
        return ContactsEmployeeAppointmentsAnswerGapFinding(
            ...
            rationale=(
                "Core Contacts / Employee Appointments support exists but may need clearer answer synthesis."
                if is_core
    ...
    return ContactsEmployeeAppointmentsAnswerGapFinding(
        ...
        rationale=(
            "This is a core Contacts / Employee Appointments concept and missing evidence will materially weaken answers."
            if is_core
...
def _overall_status(findings: list[ContactsEmployeeAppointmentsAnswerGapFinding]) -> str:
    core_findings = [
        finding for finding in findings if finding.group_key in CORE_CONTACTS_EMPLOYEE_APPOINTMENTS_GROUPS
    ]
...
def _recommended_next_actions(
    findings: list[ContactsEmployeeAppointmentsAnswerGapFinding], overall_status: str
) -> list[str]:
    actions: list[str] = []
    if overall_status == "GOOD":
        actions.append(
            "Keep current Contacts / Employee Appointments retrieval terms and answer synthesis under benchmark watch."
        )
    if any(f.recommended_action == "ADD_FORMAL_SOURCE_EVIDENCE_LATER" for f in findings):
        actions.append(
            "Add formal source evidence later for missing Contacts / Employee Appointments groups before widening answer claims."
        )
    if any(f.recommended_action == "IMPROVE_RETRIEVAL_TERMS" for f in findings):
        actions.append(
            "Refine Contacts / Employee Appointments retrieval terms for weak supporting groups before adding new corpus."
        )
    if any(f.recommended_action == "IMPROVE_SYNTHESIS" for f in findings):
        actions.append(
            "Tighten Contacts / Employee Appointments answer synthesis for weak core groups while keeping status caveats."
        )
    if overall_status == "INSUFFICIENT_CORPUS":
        actions.append(
            "Treat Contacts / Employee Appointments answers as corpus-limited until core evidence groups are covered."
        )
    return list(dict.fromkeys(actions))


def build_contacts_employee_appointments_answer_gap_report(
    coverage_report: dict,
) -> ContactsEmployeeAppointmentsAnswerGapReport:
    validate_contacts_employee_appointments_coverage_report(coverage_report)
    findings = [_finding_for_group(group) for group in coverage_report["groups"]]
    overall_status = _overall_status(findings)
    return ContactsEmployeeAppointmentsAnswerGapReport(
        report_type="CONTACTS_EMPLOYEE_APPOINTMENTS_ANSWER_GAP_REPORT",
