...
CORE_FINALISATION_READINESS_GROUPS = {
...
class FinalisationReadinessAnswerGapReportError(ValueError):
    pass


@dataclass(frozen=True)
class FinalisationReadinessAnswerGapFinding:
    group_key: str
    coverage_status: str
    answer_impact: str
...
@dataclass(frozen=True)
class FinalisationReadinessAnswerGapReport:
    report_type: str
    generated_at_utc: str
    overall_status: str
    group_findings: list[FinalisationReadinessAnswerGapFinding]
...
def validate_finalisation_readiness_coverage_report(coverage_report: dict) -> None:
    if not isinstance(coverage_report, dict):
        raise FinalisationReadinessAnswerGapReportError("Coverage report must be a JSON object.")
    if coverage_report.get("plan_id") != "FINALISATION_READINESS":
        raise FinalisationReadinessAnswerGapReportError(
            "Coverage report must have plan_id FINALISATION_READINESS."
        )
    if not isinstance(coverage_report.get("groups"), list):
        raise FinalisationReadinessAnswerGapReportError("Coverage report must include groups[].")
    if not coverage_report["groups"]:
        raise FinalisationReadinessAnswerGapReportError("Coverage report groups[] must not be empty.")

    for index, group in enumerate(coverage_report["groups"]):
        if not isinstance(group, dict):
            raise FinalisationReadinessAnswerGapReportError(f"Coverage group at index {index} must be an object.")
        group_key = group.get("group_key")
        coverage_status = group.get("coverage_status")
        if not group_key:
            raise FinalisationReadinessAnswerGapReportError(f"Coverage group at index {index} is missing group_key.")
...
def _finding_for_group(group: dict) -> FinalisationReadinessAnswerGapFinding:
    group_key = group["group_key"]
    status = group["coverage_status"]
    is_core = group_key in CORE_FINALISATION_READINESS_GROUPS

    if status == "STRONG":
        return FinalisationReadinessAnswerGapFinding(
            group_key=group_key,
            coverage_status=status,
            answer_impact="LOW",
            diagnostic_summary=_diagnostic_summary(group),
            recommended_action="KEEP",
            rationale="Formal-corpus coverage is strong enough for the current Finalisation Readiness answer standard.",
        )
    if status == "WEAK":
        ...
        return FinalisationReadinessAnswerGapFinding(
            ...
            rationale=(
                "Core Finalisation Readiness support exists but may need clearer answer synthesis."
                if is_core
    ...
    return FinalisationReadinessAnswerGapFinding(
        ...
        rationale=(
            "This is a core Finalisation Readiness concept and missing evidence will materially weaken answers."
            if is_core
...
def _overall_status(findings: list[FinalisationReadinessAnswerGapFinding]) -> str:
    core_findings = [
        finding for finding in findings if finding.group_key in CORE_FINALISATION_READINESS_GROUPS
    ]
...
def _recommended_next_actions(
    findings: list[FinalisationReadinessAnswerGapFinding],
    overall_status: str,
) -> list[str]:
    actions: list[str] = []
    if overall_status == "GOOD":
        actions.append(
            "Keep current Finalisation Readiness retrieval terms and answer synthesis under benchmark watch."
        )
    if any(f.recommended_action == "ADD_FORMAL_SOURCE_EVIDENCE_LATER" for f in findings):
        actions.append(
            "Add formal source evidence later for missing Finalisation Readiness groups before widening answer claims."
        )
    if any(f.recommended_action == "IMPROVE_RETRIEVAL_TERMS" for f in findings):
        actions.append(
            "Refine Finalisation Readiness retrieval terms for weak supporting groups before adding new corpus."
        )
    ...
    if overall_status == "INSUFFICIENT_CORPUS":
        actions.append(
            "Treat Finalisation Readiness answers as corpus-limited until core evidence groups are covered."
        )
    return list(dict.fromkeys(actions))


def build_finalisation_readiness_answer_gap_report(
    coverage_report: dict,
) -> FinalisationReadinessAnswerGapReport:
    validate_finalisation_readiness_coverage_report(coverage_report)
    findings = [_finding_for_group(group) for group in coverage_report["groups"]]
    overall_status = _overall_status(findings)
    return FinalisationReadinessAnswerGapReport(
        report_type="FINALISATION_READINESS_ANSWER_GAP_REPORT",
