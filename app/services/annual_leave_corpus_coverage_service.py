...
from app.services.domain_retrieval_plan_service import ANNUAL_LEAVE_MANAGEMENT_PLAN, EvidenceGroup
...
class AnnualLeaveGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class AnnualLeaveCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[AnnualLeaveGroupCoverage]
...
def _term_in_text(term: str, text: str) -> bool:
    return contains_normalized_term(text, term)


def _has_annual_leave_anchor(result: RetrievalResult) -> bool:
    text = f"{result.title or ''}\n{result.chunk_text}"
    return any(_term_in_text(term, text) for term in ANNUAL_LEAVE_ANCHOR_TERMS)


def _has_group_specific_required_term(group: EvidenceGroup, result: RetrievalResult) -> bool:
    text = f"{result.title or ''}\n{result.chunk_text}"
    group_specific_terms = [term for term in group.required_terms_any if term.lower() != "annual leave"]
    return any(_term_in_text(term, text) for term in group_specific_terms)


def _matched_terms(group: EvidenceGroup, results: list[RetrievalResult]) -> list[str]:
    terms = list(dict.fromkeys([*group.query_terms, *group.required_terms_any, *ANNUAL_LEAVE_ANCHOR_TERMS]))
    matched: list[str] = []
    for term in terms:
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    if status == "STRONG":
        return [
            f"Found Annual Leave support across {matched_document_count} document(s).",
            f"Matched {matched_term_count} planned Annual Leave term(s) for this evidence group.",
        ]
    if status == "WEAK":
        return [
            "Some Annual Leave formal-corpus support was found, but breadth is limited.",
            "Adjacent leave, workflow, source-model, or public-holiday evidence is not enough by itself.",
        ]
    return [
        "No useful Annual Leave-specific formal-corpus support was found for this evidence group.",
        "Do not treat adjacent leave-domain diagnostics as substitutes for this Annual Leave overview group.",
    ]
...
def scan_annual_leave_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> AnnualLeaveCoverageReport:
    group_reports: list[AnnualLeaveGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in ANNUAL_LEAVE_MANAGEMENT_PLAN.evidence_groups:
        ...
        useful_results = [
            result
            for result in results
            if _has_annual_leave_anchor(result)
            and _has_group_specific_required_term(group, result)
        ]
    ...
    return AnnualLeaveCoverageReport(
        plan_id=ANNUAL_LEAVE_MANAGEMENT_PLAN.plan_id,
        domain=ANNUAL_LEAVE_MANAGEMENT_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(ANNUAL_LEAVE_MANAGEMENT_PLAN.evidence_groups),
