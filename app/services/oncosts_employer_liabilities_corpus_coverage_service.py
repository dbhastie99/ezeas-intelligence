...
from app.services.domain_retrieval_plan_service import EvidenceGroup, ONCOSTS_EMPLOYER_LIABILITIES_PLAN
...
class OncostsEmployerLiabilitiesGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class OncostsEmployerLiabilitiesCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[OncostsEmployerLiabilitiesGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening On-costs / Employer Liabilities claims.",
        ]
    return [
...
def scan_oncosts_employer_liabilities_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> OncostsEmployerLiabilitiesCoverageReport:
    group_reports: list[OncostsEmployerLiabilitiesGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in ONCOSTS_EMPLOYER_LIABILITIES_PLAN.evidence_groups:
        ...
        group_reports.append(
            OncostsEmployerLiabilitiesGroupCoverage(
    ...
    return OncostsEmployerLiabilitiesCoverageReport(
        plan_id=ONCOSTS_EMPLOYER_LIABILITIES_PLAN.plan_id,
        domain=ONCOSTS_EMPLOYER_LIABILITIES_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(ONCOSTS_EMPLOYER_LIABILITIES_PLAN.evidence_groups),
