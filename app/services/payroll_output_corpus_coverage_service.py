...
from app.services.domain_retrieval_plan_service import PAYROLL_OUTPUT_PLAN, EvidenceGroup
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


@dataclass(frozen=True)
class PayrollOutputGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class PayrollOutputCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[PayrollOutputGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Payroll Output claims.",
        ]
    return [
...
def scan_payroll_output_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> PayrollOutputCoverageReport:
    group_reports: list[PayrollOutputGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in PAYROLL_OUTPUT_PLAN.evidence_groups:
    ...
    return PayrollOutputCoverageReport(
        plan_id=PAYROLL_OUTPUT_PLAN.plan_id,
        domain=PAYROLL_OUTPUT_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(PAYROLL_OUTPUT_PLAN.evidence_groups),
