...
from app.services.domain_retrieval_plan_service import DEDUCTIONS_OBLIGATIONS_PLAN, EvidenceGroup
...
class DeductionsObligationsGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class DeductionsObligationsCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[DeductionsObligationsGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Deductions / Obligations claims.",
        ]
    return [
...
def scan_deductions_obligations_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> DeductionsObligationsCoverageReport:
    group_reports: list[DeductionsObligationsGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in DEDUCTIONS_OBLIGATIONS_PLAN.evidence_groups:
        ...
        group_reports.append(
            DeductionsObligationsGroupCoverage(
    ...
    return DeductionsObligationsCoverageReport(
        plan_id=DEDUCTIONS_OBLIGATIONS_PLAN.plan_id,
        domain=DEDUCTIONS_OBLIGATIONS_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(DEDUCTIONS_OBLIGATIONS_PLAN.evidence_groups),
