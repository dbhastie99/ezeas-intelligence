...
from app.services.domain_retrieval_plan_service import ROSTERS_PATTERNS_SCHEDULING_PLAN, EvidenceGroup
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


ROSTERS_PATTERNS_SCHEDULING_DIAGNOSTIC_GROUPS: tuple[EvidenceGroup, ...] = (
    *ROSTERS_PATTERNS_SCHEDULING_PLAN.evidence_groups,
    EvidenceGroup(
        group_id="minerva_boundaries_and_non_mutation_guardrails",
        label="Rosters / Patterns / Scheduling Minerva boundaries and non-mutation guardrails",
        query_terms=(
            "Minerva does not create rosters",
            "change worker schedules",
            "mutate Pattern PatternDay EmployeeAppointmentPattern truth",
            "mutate ObjectTime",
            "calculate payroll",
            "decide entitlements",
            "calculate leave",
            ...
            "finalise PayRuns",
            "mutate operational workforce payroll leave truth",
        ),
        required_terms_any=(
            "Minerva does not create rosters",
            "change worker schedules",
...
class RostersPatternsSchedulingGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class RostersPatternsSchedulingCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[RostersPatternsSchedulingGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Rosters / Patterns / Scheduling claims.",
        ]
    return [
...
def scan_rosters_patterns_scheduling_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> RostersPatternsSchedulingCoverageReport:
    group_reports: list[RostersPatternsSchedulingGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in ROSTERS_PATTERNS_SCHEDULING_DIAGNOSTIC_GROUPS:
        ...
        group_reports.append(
            RostersPatternsSchedulingGroupCoverage(
    ...
    return RostersPatternsSchedulingCoverageReport(
        plan_id=ROSTERS_PATTERNS_SCHEDULING_PLAN.plan_id,
        domain=ROSTERS_PATTERNS_SCHEDULING_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(ROSTERS_PATTERNS_SCHEDULING_DIAGNOSTIC_GROUPS),
