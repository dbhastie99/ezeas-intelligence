...
from app.services.domain_retrieval_plan_service import EvidenceGroup, PAYRUN_ADMIN_QUEUE_PLAN
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


@dataclass(frozen=True)
class PayRunAdminQueueGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class PayRunAdminQueueCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[PayRunAdminQueueGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening PayRun Admin Queue claims.",
        ]
    return [
...
def scan_payrun_admin_queue_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> PayRunAdminQueueCoverageReport:
    group_reports: list[PayRunAdminQueueGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in PAYRUN_ADMIN_QUEUE_PLAN.evidence_groups:
    ...
    return PayRunAdminQueueCoverageReport(
        plan_id=PAYRUN_ADMIN_QUEUE_PLAN.plan_id,
        domain=PAYRUN_ADMIN_QUEUE_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(PAYRUN_ADMIN_QUEUE_PLAN.evidence_groups),
