...
from app.services.domain_retrieval_plan_service import EvidenceGroup, PAYROLL_BASES_AND_TOTALS_PLAN
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


@dataclass(frozen=True)
class PayrollBasesGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class PayrollBasesCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[PayrollBasesGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Payroll Bases & Totals claims.",
        ]
    return [
...
def scan_payroll_bases_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> PayrollBasesCoverageReport:
    group_reports: list[PayrollBasesGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in PAYROLL_BASES_AND_TOTALS_PLAN.evidence_groups:
    ...
    return PayrollBasesCoverageReport(
        plan_id=PAYROLL_BASES_AND_TOTALS_PLAN.plan_id,
        domain=PAYROLL_BASES_AND_TOTALS_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(PAYROLL_BASES_AND_TOTALS_PLAN.evidence_groups),
