...
from app.services.domain_retrieval_plan_service import EvidenceGroup, MOVEMENT_REVIEW_PLAN
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


@dataclass(frozen=True)
class MovementReviewGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class MovementReviewCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[MovementReviewGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Movement Review claims.",
        ]
    return [
...
def scan_movement_review_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> MovementReviewCoverageReport:
    group_reports: list[MovementReviewGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in MOVEMENT_REVIEW_PLAN.evidence_groups:
    ...
    return MovementReviewCoverageReport(
        plan_id=MOVEMENT_REVIEW_PLAN.plan_id,
        domain=MOVEMENT_REVIEW_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(MOVEMENT_REVIEW_PLAN.evidence_groups),
