...
PAYROLL_TAX_WORKCOVER_WIC_LIABILITY_DETAIL_DIAGNOSTIC_GROUPS: tuple[EvidenceGroup, ...] = (
    *PAYROLL_TAX_WORKCOVER_WIC_LIABILITY_DETAIL_PLAN.evidence_groups,
    EvidenceGroup(
        group_id="minerva_boundaries_and_non_mutation_guardrails",
        label="Payroll Tax / WorkCover / WIC Minerva boundaries and non-mutation guardrails",
        query_terms=(
            "Minerva does not calculate payroll tax",
            "does not calculate WorkCover or WIC liability",
            "does not lodge or remit statutory returns",
            "does not decide statutory liability",
            "does not change employer-liability configuration",
            "does not change Worksite State jurisdiction truth",
            "does not calculate payroll",
            "does not mutate payroll output",
            "does not determine finalisation readiness",
            "does not finalise PayRuns",
            "does not mutate operational workforce payroll liability truth",
        ),
        required_terms_any=(
            "Minerva does not calculate payroll tax",
            "does not calculate WorkCover",
            "does not lodge or remit statutory returns",
            "does not decide statutory liability",
            "does not change employer-liability configuration",
            "does not mutate operational workforce",
        ),
...
class PayrollTaxWorkcoverWicLiabilityDetailGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class PayrollTaxWorkcoverWicLiabilityDetailCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[PayrollTaxWorkcoverWicLiabilityDetailGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Payroll Tax / WorkCover / WIC liability-detail claims.",
        ]
    return [
...
def scan_payroll_tax_workcover_wic_liability_detail_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> PayrollTaxWorkcoverWicLiabilityDetailCoverageReport:
    group_reports: list[PayrollTaxWorkcoverWicLiabilityDetailGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in PAYROLL_TAX_WORKCOVER_WIC_LIABILITY_DETAIL_DIAGNOSTIC_GROUPS:
        ...
        group_reports.append(
            PayrollTaxWorkcoverWicLiabilityDetailGroupCoverage(
    ...
    return PayrollTaxWorkcoverWicLiabilityDetailCoverageReport(
        plan_id=PAYROLL_TAX_WORKCOVER_WIC_LIABILITY_DETAIL_PLAN.plan_id,
        domain=PAYROLL_TAX_WORKCOVER_WIC_LIABILITY_DETAIL_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(PAYROLL_TAX_WORKCOVER_WIC_LIABILITY_DETAIL_DIAGNOSTIC_GROUPS),
