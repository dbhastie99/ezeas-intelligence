...
CORE_AWARD_BUILD_EVIDENCE_GROUPS = {
...
class AwardBuildEvidenceAnswerGapReportError(ValueError):
    pass


@dataclass(frozen=True)
class AwardBuildEvidenceAnswerGapFinding:
    group_key: str
    coverage_status: str
    answer_impact: str
...
@dataclass(frozen=True)
class AwardBuildEvidenceAnswerGapReport:
    report_type: str
    generated_at_utc: str
    overall_status: str
    group_findings: list[AwardBuildEvidenceAnswerGapFinding]
...
def validate_award_build_evidence_coverage_report(coverage_report: dict) -> None:
    if not isinstance(coverage_report, dict):
        raise AwardBuildEvidenceAnswerGapReportError("Coverage report must be a JSON object.")
    if coverage_report.get("plan_id") != "AWARD_BUILD_EVIDENCE":
        raise AwardBuildEvidenceAnswerGapReportError("Coverage report must have plan_id AWARD_BUILD_EVIDENCE.")
    if not isinstance(coverage_report.get("groups"), list):
        raise AwardBuildEvidenceAnswerGapReportError("Coverage report must include groups[].")
    if not coverage_report["groups"]:
        raise AwardBuildEvidenceAnswerGapReportError("Coverage report groups[] must not be empty.")

    for index, group in enumerate(coverage_report["groups"]):
        if not isinstance(group, dict):
            raise AwardBuildEvidenceAnswerGapReportError(f"Coverage group at index {index} must be an object.")
        group_key = group.get("group_key")
        coverage_status = group.get("coverage_status")
        if not group_key:
            raise AwardBuildEvidenceAnswerGapReportError(f"Coverage group at index {index} is missing group_key.")
        if coverage_status not in VALID_COVERAGE_STATUSES:
            raise AwardBuildEvidenceAnswerGapReportError(
...
def _finding_for_group(group: dict) -> AwardBuildEvidenceAnswerGapFinding:
    group_key = group["group_key"]
    status = group["coverage_status"]
    is_core = group_key in CORE_AWARD_BUILD_EVIDENCE_GROUPS

    if status == "STRONG":
        return AwardBuildEvidenceAnswerGapFinding(
            group_key=group_key,
            ...
            rationale=(
                "Formal-corpus coverage is strong enough for the current Award Build / Award Evidence answer standard."
            ),
        )
    if status == "WEAK":
        ...
        return AwardBuildEvidenceAnswerGapFinding(
            ...
            rationale=(
                "Core Award Build / Award Evidence support exists but may need clearer answer synthesis."
                if is_core
    ...
    return AwardBuildEvidenceAnswerGapFinding(
        ...
        rationale=(
            "This is a core Award Build / Award Evidence concept and missing evidence will materially weaken answers."
            if is_core
...
def _overall_status(findings: list[AwardBuildEvidenceAnswerGapFinding]) -> str:
    core_findings = [finding for finding in findings if finding.group_key in CORE_AWARD_BUILD_EVIDENCE_GROUPS]
...
def _recommended_next_actions(
    findings: list[AwardBuildEvidenceAnswerGapFinding],
    overall_status: str,
) -> list[str]:
    actions: list[str] = []
    if overall_status == "GOOD":
        actions.append("Keep current Award Build / Award Evidence retrieval terms and answer synthesis under benchmark watch.")
    if any(f.recommended_action == "ADD_FORMAL_SOURCE_EVIDENCE_LATER" for f in findings):
        actions.append(
            "Add formal source evidence later for missing Award Build / Award Evidence groups before widening answer claims."
        )
    if any(f.recommended_action == "IMPROVE_RETRIEVAL_TERMS" for f in findings):
        actions.append(
            "Refine Award Build / Award Evidence retrieval terms for weak supporting groups before adding new corpus."
        )
    if any(f.recommended_action == "IMPROVE_SYNTHESIS" for f in findings):
        actions.append(
            "Tighten Award Build / Award Evidence answer synthesis for weak core groups while keeping status caveats."
        )
    if overall_status == "INSUFFICIENT_CORPUS":
        actions.append("Treat Award Build / Award Evidence answers as corpus-limited until core evidence groups are covered.")
    return list(dict.fromkeys(actions))


def build_award_build_evidence_answer_gap_report(
    coverage_report: dict,
) -> AwardBuildEvidenceAnswerGapReport:
    validate_award_build_evidence_coverage_report(coverage_report)
    findings = [_finding_for_group(group) for group in coverage_report["groups"]]
    overall_status = _overall_status(findings)
    return AwardBuildEvidenceAnswerGapReport(
        report_type="AWARD_BUILD_EVIDENCE_ANSWER_GAP_REPORT",
