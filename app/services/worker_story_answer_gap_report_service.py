...
class WorkerStoryAnswerGapReportError(ValueError):
    pass


@dataclass(frozen=True)
class WorkerStoryAnswerGapFinding:
    group_key: str
    coverage_status: str
    answer_impact: str
...
@dataclass(frozen=True)
class WorkerStoryAnswerGapReport:
    report_type: str
    generated_at_utc: str
    overall_status: str
    group_findings: list[WorkerStoryAnswerGapFinding]
...
def validate_worker_story_coverage_report(coverage_report: dict) -> None:
    if not isinstance(coverage_report, dict):
        raise WorkerStoryAnswerGapReportError("Coverage report must be a JSON object.")
    if coverage_report.get("plan_id") != "WORKER_STORY":
        raise WorkerStoryAnswerGapReportError("Coverage report must have plan_id WORKER_STORY.")
    if not isinstance(coverage_report.get("groups"), list):
        raise WorkerStoryAnswerGapReportError("Coverage report must include groups[].")
    if not coverage_report["groups"]:
        raise WorkerStoryAnswerGapReportError("Coverage report groups[] must not be empty.")

    for index, group in enumerate(coverage_report["groups"]):
        if not isinstance(group, dict):
            raise WorkerStoryAnswerGapReportError(f"Coverage group at index {index} must be an object.")
        group_key = group.get("group_key")
        coverage_status = group.get("coverage_status")
        if not group_key:
            raise WorkerStoryAnswerGapReportError(f"Coverage group at index {index} is missing group_key.")
        if coverage_status not in VALID_COVERAGE_STATUSES:
            raise WorkerStoryAnswerGapReportError(
...
def _finding_for_group(group: dict) -> WorkerStoryAnswerGapFinding:
    group_key = group["group_key"]
    status = group["coverage_status"]
    is_core = group_key in CORE_WORKER_STORY_GROUPS

    if status == "STRONG":
        return WorkerStoryAnswerGapFinding(
            group_key=group_key,
            coverage_status=status,
            answer_impact="LOW",
            diagnostic_summary=_diagnostic_summary(group),
            recommended_action="KEEP",
            rationale="Formal-corpus coverage is strong enough for the current answer standard.",
        )
    if status == "WEAK":
        ...
        return WorkerStoryAnswerGapFinding(
            ...
            rationale=(
                "Core Worker Story support exists but may need clearer answer synthesis."
                if is_core
    ...
    return WorkerStoryAnswerGapFinding(
        ...
        rationale=(
            "This is a core Worker Story concept and missing evidence will materially weaken answers."
            if is_core
...
def _overall_status(findings: list[WorkerStoryAnswerGapFinding]) -> str:
    core_findings = [finding for finding in findings if finding.group_key in CORE_WORKER_STORY_GROUPS]
...
def _recommended_next_actions(findings: list[WorkerStoryAnswerGapFinding], overall_status: str) -> list[str]:
    actions: list[str] = []
    if overall_status == "GOOD":
        actions.append("Keep current Worker Story retrieval terms and answer synthesis under benchmark watch.")
    if any(f.recommended_action == "ADD_FORMAL_SOURCE_EVIDENCE_LATER" for f in findings):
        actions.append("Add formal source evidence later for missing Worker Story groups before widening answer claims.")
    if any(f.recommended_action == "IMPROVE_RETRIEVAL_TERMS" for f in findings):
        actions.append("Refine Worker Story retrieval terms for weak supporting groups before adding new corpus.")
    if any(f.recommended_action == "IMPROVE_SYNTHESIS" for f in findings):
        actions.append("Tighten Worker Story answer synthesis for weak core groups while keeping status caveats.")
    if overall_status == "INSUFFICIENT_CORPUS":
        actions.append("Treat Worker Story answers as corpus-limited until core evidence groups are covered.")
    return list(dict.fromkeys(actions))


def build_worker_story_answer_gap_report(coverage_report: dict) -> WorkerStoryAnswerGapReport:
    validate_worker_story_coverage_report(coverage_report)
    findings = [_finding_for_group(group) for group in coverage_report["groups"]]
    overall_status = _overall_status(findings)
    return WorkerStoryAnswerGapReport(
        report_type="WORKER_STORY_ANSWER_GAP_REPORT",
