...
from app.services.domain_retrieval_plan_service import AWARD_BUILD_EVIDENCE_PLAN, EvidenceGroup
...
class AwardBuildEvidenceGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class AwardBuildEvidenceCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[AwardBuildEvidenceGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Award Build / Award Evidence claims.",
        ]
    return [
...
def scan_award_build_evidence_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> AwardBuildEvidenceCoverageReport:
    group_reports: list[AwardBuildEvidenceGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in AWARD_BUILD_EVIDENCE_PLAN.evidence_groups:
        ...
        group_reports.append(
            AwardBuildEvidenceGroupCoverage(
    ...
    return AwardBuildEvidenceCoverageReport(
        plan_id=AWARD_BUILD_EVIDENCE_PLAN.plan_id,
        domain=AWARD_BUILD_EVIDENCE_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(AWARD_BUILD_EVIDENCE_PLAN.evidence_groups),
