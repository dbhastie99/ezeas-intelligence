...
from app.services.domain_retrieval_plan_service import PUBLIC_HOLIDAYS_PLAN, EvidenceGroup
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


PUBLIC_HOLIDAYS_DIAGNOSTIC_GROUPS: tuple[EvidenceGroup, ...] = (
    *PUBLIC_HOLIDAYS_PLAN.evidence_groups,
    EvidenceGroup(
        group_id="minerva_boundaries_and_non_mutation_guardrails",
        label="Public Holidays Minerva boundaries and non-mutation guardrails",
        query_terms=(
            ...
            "change PublicHolidayGroup configuration",
            "mutate Worksite EmployeeAppointment PayRun LeaveRequest truth",
...
class PublicHolidaysGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class PublicHolidaysCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[PublicHolidaysGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Public Holidays claims.",
        ]
    return [
...
def scan_public_holidays_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> PublicHolidaysCoverageReport:
    group_reports: list[PublicHolidaysGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in PUBLIC_HOLIDAYS_DIAGNOSTIC_GROUPS:
    ...
    return PublicHolidaysCoverageReport(
        plan_id=PUBLIC_HOLIDAYS_PLAN.plan_id,
        domain=PUBLIC_HOLIDAYS_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(PUBLIC_HOLIDAYS_DIAGNOSTIC_GROUPS),
