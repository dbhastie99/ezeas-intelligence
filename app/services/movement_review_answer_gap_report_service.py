...
class MovementReviewAnswerGapReportError(ValueError):
    pass


@dataclass(frozen=True)
class MovementReviewAnswerGapFinding:
    group_key: str
    coverage_status: str
    answer_impact: str
...
@dataclass(frozen=True)
class MovementReviewAnswerGapReport:
    report_type: str
    generated_at_utc: str
    overall_status: str
    group_findings: list[MovementReviewAnswerGapFinding]
...
def validate_movement_review_coverage_report(coverage_report: dict) -> None:
    if not isinstance(coverage_report, dict):
        raise MovementReviewAnswerGapReportError("Coverage report must be a JSON object.")
    if coverage_report.get("plan_id") != "MOVEMENT_REVIEW":
        raise MovementReviewAnswerGapReportError("Coverage report must have plan_id MOVEMENT_REVIEW.")
    if not isinstance(coverage_report.get("groups"), list):
        raise MovementReviewAnswerGapReportError("Coverage report must include groups[].")
    if not coverage_report["groups"]:
        raise MovementReviewAnswerGapReportError("Coverage report groups[] must not be empty.")

    for index, group in enumerate(coverage_report["groups"]):
        if not isinstance(group, dict):
            raise MovementReviewAnswerGapReportError(f"Coverage group at index {index} must be an object.")
        group_key = group.get("group_key")
        coverage_status = group.get("coverage_status")
        if not group_key:
            raise MovementReviewAnswerGapReportError(f"Coverage group at index {index} is missing group_key.")
        if coverage_status not in VALID_COVERAGE_STATUSES:
            raise MovementReviewAnswerGapReportError(
...
def _finding_for_group(group: dict) -> MovementReviewAnswerGapFinding:
    group_key = group["group_key"]
    status = group["coverage_status"]
    is_core = group_key in CORE_MOVEMENT_REVIEW_GROUPS

    if status == "STRONG":
        return MovementReviewAnswerGapFinding(
            group_key=group_key,
            coverage_status=status,
            answer_impact="LOW",
            diagnostic_summary=_diagnostic_summary(group),
            recommended_action="KEEP",
            rationale="Formal-corpus coverage is strong enough for the current Movement Review answer standard.",
        )
    if status == "WEAK":
        ...
        return MovementReviewAnswerGapFinding(
            ...
            rationale=(
                "Core Movement Review support exists but may need clearer answer synthesis."
                if is_core
    ...
    return MovementReviewAnswerGapFinding(
        ...
        rationale=(
            "This is a core Movement Review concept and missing evidence will materially weaken answers."
            if is_core
...
def _overall_status(findings: list[MovementReviewAnswerGapFinding]) -> str:
    core_findings = [finding for finding in findings if finding.group_key in CORE_MOVEMENT_REVIEW_GROUPS]
...
def _recommended_next_actions(findings: list[MovementReviewAnswerGapFinding], overall_status: str) -> list[str]:
    actions: list[str] = []
    if overall_status == "GOOD":
        actions.append("Keep current Movement Review retrieval terms and answer synthesis under benchmark watch.")
    if any(f.recommended_action == "ADD_FORMAL_SOURCE_EVIDENCE_LATER" for f in findings):
        actions.append("Add formal source evidence later for missing Movement Review groups before widening answer claims.")
    if any(f.recommended_action == "IMPROVE_RETRIEVAL_TERMS" for f in findings):
        actions.append("Refine Movement Review retrieval terms for weak supporting groups before adding new corpus.")
    if any(f.recommended_action == "IMPROVE_SYNTHESIS" for f in findings):
        actions.append("Tighten Movement Review answer synthesis for weak core groups while keeping status caveats.")
    if overall_status == "INSUFFICIENT_CORPUS":
        actions.append("Treat Movement Review answers as corpus-limited until core evidence groups are covered.")
    return list(dict.fromkeys(actions))


def build_movement_review_answer_gap_report(coverage_report: dict) -> MovementReviewAnswerGapReport:
    validate_movement_review_coverage_report(coverage_report)
    findings = [_finding_for_group(group) for group in coverage_report["groups"]]
    overall_status = _overall_status(findings)
    return MovementReviewAnswerGapReport(
        report_type="MOVEMENT_REVIEW_ANSWER_GAP_REPORT",
