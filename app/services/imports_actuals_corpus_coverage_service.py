...
from app.services.domain_retrieval_plan_service import IMPORTS_ACTUALS_PLAN, EvidenceGroup
...
class ImportsActualsGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class ImportsActualsCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[ImportsActualsGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Imports / Actuals claims.",
        ]
    return [
...
def scan_imports_actuals_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> ImportsActualsCoverageReport:
    group_reports: list[ImportsActualsGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in IMPORTS_ACTUALS_PLAN.evidence_groups:
    ...
    return ImportsActualsCoverageReport(
        plan_id=IMPORTS_ACTUALS_PLAN.plan_id,
        domain=IMPORTS_ACTUALS_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(IMPORTS_ACTUALS_PLAN.evidence_groups),
