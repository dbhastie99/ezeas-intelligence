...
from app.services.domain_retrieval_plan_service import EvidenceGroup, TAX_PAYG_PLAN
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


@dataclass(frozen=True)
class TaxPaygGroupCoverage:
    group_key: str
    group_label: str
...
class TaxPaygCoverageReport:
    ...
    groups: list[TaxPaygGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Tax / PAYG claims.",
        ]
    return [
...
def scan_tax_payg_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> TaxPaygCoverageReport:
    group_reports: list[TaxPaygGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in TAX_PAYG_PLAN.evidence_groups:
    ...
    return TaxPaygCoverageReport(
        plan_id=TAX_PAYG_PLAN.plan_id,
        domain=TAX_PAYG_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(TAX_PAYG_PLAN.evidence_groups),
