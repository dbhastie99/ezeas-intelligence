...
from app.services.domain_retrieval_plan_service import GROSS_TO_NET_PLAN, EvidenceGroup
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


@dataclass(frozen=True)
class GrossToNetGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class GrossToNetCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[GrossToNetGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Gross-to-Net claims.",
        ]
    return [
...
def scan_gross_to_net_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> GrossToNetCoverageReport:
    group_reports: list[GrossToNetGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in GROSS_TO_NET_PLAN.evidence_groups:
    ...
    return GrossToNetCoverageReport(
        plan_id=GROSS_TO_NET_PLAN.plan_id,
        domain=GROSS_TO_NET_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(GROSS_TO_NET_PLAN.evidence_groups),
