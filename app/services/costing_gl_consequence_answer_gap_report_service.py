...
CORE_COSTING_GL_CONSEQUENCE_GROUPS = {
...
class CostingGlConsequenceAnswerGapReportError(ValueError):
    pass


@dataclass(frozen=True)
class CostingGlConsequenceAnswerGapFinding:
    group_key: str
    coverage_status: str
    answer_impact: str
...
@dataclass(frozen=True)
class CostingGlConsequenceAnswerGapReport:
    report_type: str
    generated_at_utc: str
    overall_status: str
    group_findings: list[CostingGlConsequenceAnswerGapFinding]
...
def validate_costing_gl_consequence_coverage_report(coverage_report: dict) -> None:
    if not isinstance(coverage_report, dict):
        raise CostingGlConsequenceAnswerGapReportError("Coverage report must be a JSON object.")
    if coverage_report.get("plan_id") != "COSTING_GL_CONSEQUENCE":
        raise CostingGlConsequenceAnswerGapReportError("Coverage report must have plan_id COSTING_GL_CONSEQUENCE.")
    if not isinstance(coverage_report.get("groups"), list):
        raise CostingGlConsequenceAnswerGapReportError("Coverage report must include groups[].")
    if not coverage_report["groups"]:
        raise CostingGlConsequenceAnswerGapReportError("Coverage report groups[] must not be empty.")

    for index, group in enumerate(coverage_report["groups"]):
        if not isinstance(group, dict):
            raise CostingGlConsequenceAnswerGapReportError(f"Coverage group at index {index} must be an object.")
        group_key = group.get("group_key")
        coverage_status = group.get("coverage_status")
        if not group_key:
            raise CostingGlConsequenceAnswerGapReportError(
...
def _finding_for_group(group: dict) -> CostingGlConsequenceAnswerGapFinding:
    group_key = group["group_key"]
    status = group["coverage_status"]
    is_core = group_key in CORE_COSTING_GL_CONSEQUENCE_GROUPS

    if status == "STRONG":
        return CostingGlConsequenceAnswerGapFinding(
            group_key=group_key,
            ...
            rationale=(
                "Formal-corpus coverage is strong enough for the current Costing / GL Consequence Evidence "
                "answer standard."
            ),
        )
    if status == "WEAK":
        ...
        return CostingGlConsequenceAnswerGapFinding(
            ...
            rationale=(
                "Core Costing / GL Consequence support exists but may need clearer answer synthesis."
                if is_core
    ...
    return CostingGlConsequenceAnswerGapFinding(
        ...
        rationale=(
            "This is a core Costing / GL Consequence concept and missing evidence will materially weaken answers."
            if is_core
...
def _overall_status(findings: list[CostingGlConsequenceAnswerGapFinding]) -> str:
    core_findings = [finding for finding in findings if finding.group_key in CORE_COSTING_GL_CONSEQUENCE_GROUPS]
...
def _recommended_next_actions(
    findings: list[CostingGlConsequenceAnswerGapFinding], overall_status: str
) -> list[str]:
    actions: list[str] = []
    if overall_status == "GOOD":
        actions.append(
            "Keep current Costing / GL Consequence Evidence retrieval terms and answer synthesis under benchmark watch."
        )
    if any(f.recommended_action == "ADD_FORMAL_SOURCE_EVIDENCE_LATER" for f in findings):
        actions.append(
            "Add formal source evidence later for missing Costing / GL Consequence groups before widening answer claims."
        )
    if any(f.recommended_action == "IMPROVE_RETRIEVAL_TERMS" for f in findings):
        actions.append(
            "Refine Costing / GL Consequence retrieval terms for weak supporting groups before adding new corpus."
        )
    if any(f.recommended_action == "IMPROVE_SYNTHESIS" for f in findings):
        actions.append(
            "Tighten Costing / GL Consequence answer synthesis for weak core groups while keeping status caveats."
        )
    if overall_status == "INSUFFICIENT_CORPUS":
        actions.append(
            "Treat Costing / GL Consequence Evidence answers as corpus-limited until core evidence groups are covered."
        )
    return list(dict.fromkeys(actions))


def build_costing_gl_consequence_answer_gap_report(
    coverage_report: dict,
) -> CostingGlConsequenceAnswerGapReport:
    validate_costing_gl_consequence_coverage_report(coverage_report)
    findings = [_finding_for_group(group) for group in coverage_report["groups"]]
    overall_status = _overall_status(findings)
    return CostingGlConsequenceAnswerGapReport(
        report_type="COSTING_GL_CONSEQUENCE_ANSWER_GAP_REPORT",
