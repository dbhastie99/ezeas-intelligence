...
from app.services.domain_retrieval_plan_service import COSTING_GL_CONSEQUENCE_PLAN, EvidenceGroup
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


@dataclass(frozen=True)
class CostingGlConsequenceGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class CostingGlConsequenceCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[CostingGlConsequenceGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Costing / GL Consequence Evidence claims.",
        ]
    return [
...
def scan_costing_gl_consequence_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> CostingGlConsequenceCoverageReport:
    group_reports: list[CostingGlConsequenceGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in COSTING_GL_CONSEQUENCE_PLAN.evidence_groups:
        ...
        group_reports.append(
            CostingGlConsequenceGroupCoverage(
    ...
    return CostingGlConsequenceCoverageReport(
        plan_id=COSTING_GL_CONSEQUENCE_PLAN.plan_id,
        domain=COSTING_GL_CONSEQUENCE_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(COSTING_GL_CONSEQUENCE_PLAN.evidence_groups),
