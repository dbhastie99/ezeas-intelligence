...
from app.services.domain_retrieval_plan_service import RATE_SOURCE_RATE_STORY_PLAN, EvidenceGroup
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


@dataclass(frozen=True)
class RateSourceRateStoryGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class RateSourceRateStoryCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[RateSourceRateStoryGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening RateSource / Rate Story claims.",
        ]
    return [
...
def scan_rate_source_rate_story_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> RateSourceRateStoryCoverageReport:
    group_reports: list[RateSourceRateStoryGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in RATE_SOURCE_RATE_STORY_PLAN.evidence_groups:
        ...
        group_reports.append(
            RateSourceRateStoryGroupCoverage(
    ...
    return RateSourceRateStoryCoverageReport(
        plan_id=RATE_SOURCE_RATE_STORY_PLAN.plan_id,
        domain=RATE_SOURCE_RATE_STORY_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(RATE_SOURCE_RATE_STORY_PLAN.evidence_groups),
