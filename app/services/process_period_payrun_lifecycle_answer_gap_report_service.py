...
CORE_PROCESS_PERIOD_PAYRUN_LIFECYCLE_GROUPS = {
...
class ProcessPeriodPayRunLifecycleAnswerGapReportError(ValueError):
    pass


@dataclass(frozen=True)
class ProcessPeriodPayRunLifecycleAnswerGapFinding:
    group_key: str
    coverage_status: str
    answer_impact: str
...
@dataclass(frozen=True)
class ProcessPeriodPayRunLifecycleAnswerGapReport:
    report_type: str
    generated_at_utc: str
    overall_status: str
    group_findings: list[ProcessPeriodPayRunLifecycleAnswerGapFinding]
...
def validate_process_period_payrun_lifecycle_coverage_report(coverage_report: dict) -> None:
    if not isinstance(coverage_report, dict):
        raise ProcessPeriodPayRunLifecycleAnswerGapReportError("Coverage report must be a JSON object.")
    if coverage_report.get("plan_id") != "PROCESS_PERIOD_PAYRUN_LIFECYCLE":
        raise ProcessPeriodPayRunLifecycleAnswerGapReportError(
            "Coverage report must have plan_id PROCESS_PERIOD_PAYRUN_LIFECYCLE."
        )
    if not isinstance(coverage_report.get("groups"), list):
        raise ProcessPeriodPayRunLifecycleAnswerGapReportError("Coverage report must include groups[].")
    if not coverage_report["groups"]:
        raise ProcessPeriodPayRunLifecycleAnswerGapReportError("Coverage report groups[] must not be empty.")
...
def _finding_for_group(group: dict) -> ProcessPeriodPayRunLifecycleAnswerGapFinding:
    group_key = group["group_key"]
    status = group["coverage_status"]
    is_core = group_key in CORE_PROCESS_PERIOD_PAYRUN_LIFECYCLE_GROUPS

    if status == "STRONG":
        return ProcessPeriodPayRunLifecycleAnswerGapFinding(
            group_key=group_key,
            ...
            rationale=(
                "Formal-corpus coverage is strong enough for the current Process Periods / PayRun Lifecycle "
                "answer standard."
            ),
        )
    if status == "WEAK":
        ...
        return ProcessPeriodPayRunLifecycleAnswerGapFinding(
            ...
            rationale=(
                "Core Process Periods / PayRun Lifecycle support exists but may need clearer answer synthesis."
                if is_core
    ...
    return ProcessPeriodPayRunLifecycleAnswerGapFinding(
        ...
        rationale=(
            "This is a core Process Periods / PayRun Lifecycle concept and missing evidence will materially weaken answers."
            if is_core
...
def _overall_status(findings: list[ProcessPeriodPayRunLifecycleAnswerGapFinding]) -> str:
    core_findings = [finding for finding in findings if finding.group_key in CORE_PROCESS_PERIOD_PAYRUN_LIFECYCLE_GROUPS]
...
def _recommended_next_actions(
    findings: list[ProcessPeriodPayRunLifecycleAnswerGapFinding], overall_status: str
) -> list[str]:
    actions: list[str] = []
    ...
    if any(f.recommended_action == "ADD_FORMAL_SOURCE_EVIDENCE_LATER" for f in findings):
        actions.append(
            "Add formal source evidence later for missing Process Periods / PayRun Lifecycle groups before widening answer claims."
        )
    ...
    if overall_status == "INSUFFICIENT_CORPUS":
        actions.append(
            "Treat Process Periods / PayRun Lifecycle answers as corpus-limited until core evidence groups are covered."
        )
    return list(dict.fromkeys(actions))


def build_process_period_payrun_lifecycle_answer_gap_report(
    coverage_report: dict,
) -> ProcessPeriodPayRunLifecycleAnswerGapReport:
    validate_process_period_payrun_lifecycle_coverage_report(coverage_report)
    findings = [_finding_for_group(group) for group in coverage_report["groups"]]
    overall_status = _overall_status(findings)
    return ProcessPeriodPayRunLifecycleAnswerGapReport(
        report_type="PROCESS_PERIOD_PAYRUN_LIFECYCLE_ANSWER_GAP_REPORT",
