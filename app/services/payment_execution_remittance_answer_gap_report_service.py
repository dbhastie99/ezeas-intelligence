...
CORE_PAYMENT_EXECUTION_REMITTANCE_GROUPS = {
...
class PaymentExecutionRemittanceAnswerGapReportError(ValueError):
    pass


@dataclass(frozen=True)
class PaymentExecutionRemittanceAnswerGapFinding:
    group_key: str
    coverage_status: str
    answer_impact: str
...
@dataclass(frozen=True)
class PaymentExecutionRemittanceAnswerGapReport:
    report_type: str
    generated_at_utc: str
    overall_status: str
    group_findings: list[PaymentExecutionRemittanceAnswerGapFinding]
...
def validate_payment_execution_remittance_coverage_report(coverage_report: dict) -> None:
    if not isinstance(coverage_report, dict):
        raise PaymentExecutionRemittanceAnswerGapReportError("Coverage report must be a JSON object.")
    if coverage_report.get("plan_id") != "PAYMENT_EXECUTION_REMITTANCE":
        raise PaymentExecutionRemittanceAnswerGapReportError(
            "Coverage report must have plan_id PAYMENT_EXECUTION_REMITTANCE."
        )
    if not isinstance(coverage_report.get("groups"), list):
        raise PaymentExecutionRemittanceAnswerGapReportError("Coverage report must include groups[].")
    if not coverage_report["groups"]:
        raise PaymentExecutionRemittanceAnswerGapReportError("Coverage report groups[] must not be empty.")

    for index, group in enumerate(coverage_report["groups"]):
        if not isinstance(group, dict):
            raise PaymentExecutionRemittanceAnswerGapReportError(f"Coverage group at index {index} must be an object.")
        group_key = group.get("group_key")
        coverage_status = group.get("coverage_status")
        if not group_key:
            raise PaymentExecutionRemittanceAnswerGapReportError(f"Coverage group at index {index} is missing group_key.")
...
def _finding_for_group(group: dict) -> PaymentExecutionRemittanceAnswerGapFinding:
    group_key = group["group_key"]
    status = group["coverage_status"]
    is_core = group_key in CORE_PAYMENT_EXECUTION_REMITTANCE_GROUPS

    if status == "STRONG":
        return PaymentExecutionRemittanceAnswerGapFinding(
            group_key=group_key,
            ...
            rationale=(
                "Formal-corpus coverage is strong enough for the current Payment Execution / Remittance answer standard."
            ),
        )
    if status == "WEAK":
        ...
        return PaymentExecutionRemittanceAnswerGapFinding(
            ...
            rationale=(
                "Core Payment Execution / Remittance support exists but may need clearer answer synthesis."
                if is_core
    ...
    return PaymentExecutionRemittanceAnswerGapFinding(
        ...
        rationale=(
            "This is a core Payment Execution / Remittance concept and missing evidence will materially weaken answers."
            if is_core
...
def _overall_status(findings: list[PaymentExecutionRemittanceAnswerGapFinding]) -> str:
    core_findings = [
        finding for finding in findings if finding.group_key in CORE_PAYMENT_EXECUTION_REMITTANCE_GROUPS
    ]
...
def _recommended_next_actions(
    findings: list[PaymentExecutionRemittanceAnswerGapFinding],
    overall_status: str,
) -> list[str]:
    actions: list[str] = []
    if overall_status == "GOOD":
        actions.append(
            "Keep current Payment Execution / Remittance retrieval terms and answer synthesis under benchmark watch."
        )
    if any(f.recommended_action == "ADD_FORMAL_SOURCE_EVIDENCE_LATER" for f in findings):
        actions.append(
            "Add formal source evidence later for missing Payment Execution / Remittance groups before widening answer claims."
        )
    if any(f.recommended_action == "IMPROVE_RETRIEVAL_TERMS" for f in findings):
        actions.append(
            "Refine Payment Execution / Remittance retrieval terms for weak supporting groups before adding new corpus."
        )
    if any(f.recommended_action == "IMPROVE_SYNTHESIS" for f in findings):
        actions.append(
            "Tighten Payment Execution / Remittance answer synthesis for weak core groups while keeping status caveats."
        )
    if overall_status == "INSUFFICIENT_CORPUS":
        actions.append(
            "Treat Payment Execution / Remittance answers as corpus-limited until core evidence groups are covered."
        )
    return list(dict.fromkeys(actions))


def build_payment_execution_remittance_answer_gap_report(
    coverage_report: dict,
) -> PaymentExecutionRemittanceAnswerGapReport:
    validate_payment_execution_remittance_coverage_report(coverage_report)
    findings = [_finding_for_group(group) for group in coverage_report["groups"]]
    overall_status = _overall_status(findings)
    return PaymentExecutionRemittanceAnswerGapReport(
        report_type="PAYMENT_EXECUTION_REMITTANCE_ANSWER_GAP_REPORT",
