...
from app.services.domain_retrieval_plan_service import EvidenceGroup, WORKER_STORY_PLAN
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


@dataclass(frozen=True)
class WorkerStoryGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class WorkerStoryCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[WorkerStoryGroupCoverage]
...
def _diagnostic_notes(status: str, group: EvidenceGroup, matched_term_count: int, matched_document_count: int) -> list[str]:
    if status == "STRONG":
        return [
    ...
    if status == "WEAK":
        return [
            ...
            "Add or improve formal evidence before relying on this group for richer answers.",
        ]
    return [
...
def scan_worker_story_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> WorkerStoryCoverageReport:
    group_reports: list[WorkerStoryGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in WORKER_STORY_PLAN.evidence_groups:
        ...
        group_reports.append(
            WorkerStoryGroupCoverage(
                ...
                diagnostic_notes=_diagnostic_notes(status, group, len(matched_terms), len(sources)),
            )
        )

    return WorkerStoryCoverageReport(
        plan_id=WORKER_STORY_PLAN.plan_id,
        domain=WORKER_STORY_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(WORKER_STORY_PLAN.evidence_groups),
