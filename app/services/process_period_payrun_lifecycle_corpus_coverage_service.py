...
from app.services.domain_retrieval_plan_service import PROCESS_PERIOD_PAYRUN_LIFECYCLE_PLAN, EvidenceGroup
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


@dataclass(frozen=True)
class ProcessPeriodPayRunLifecycleGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class ProcessPeriodPayRunLifecycleCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[ProcessPeriodPayRunLifecycleGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Process Periods / PayRun Lifecycle claims.",
        ]
    return [
...
def scan_process_period_payrun_lifecycle_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> ProcessPeriodPayRunLifecycleCoverageReport:
    group_reports: list[ProcessPeriodPayRunLifecycleGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in PROCESS_PERIOD_PAYRUN_LIFECYCLE_PLAN.evidence_groups:
        ...
        group_reports.append(
            ProcessPeriodPayRunLifecycleGroupCoverage(
    ...
    return ProcessPeriodPayRunLifecycleCoverageReport(
        plan_id=PROCESS_PERIOD_PAYRUN_LIFECYCLE_PLAN.plan_id,
        domain=PROCESS_PERIOD_PAYRUN_LIFECYCLE_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(PROCESS_PERIOD_PAYRUN_LIFECYCLE_PLAN.evidence_groups),
