...
CORE_AWARD_POSITIONS_CLASSIFICATIONS_GROUPS = {
...
class AwardPositionsClassificationsAnswerGapReportError(ValueError):
    pass


@dataclass(frozen=True)
class AwardPositionsClassificationsAnswerGapFinding:
    group_key: str
    coverage_status: str
    answer_impact: str
...
@dataclass(frozen=True)
class AwardPositionsClassificationsAnswerGapReport:
    report_type: str
    generated_at_utc: str
    overall_status: str
    group_findings: list[AwardPositionsClassificationsAnswerGapFinding]
...
def validate_award_positions_classifications_coverage_report(coverage_report: dict) -> None:
    if not isinstance(coverage_report, dict):
        raise AwardPositionsClassificationsAnswerGapReportError("Coverage report must be a JSON object.")
    if coverage_report.get("plan_id") != "AWARD_POSITIONS_CLASSIFICATIONS":
        raise AwardPositionsClassificationsAnswerGapReportError(
            "Coverage report must have plan_id AWARD_POSITIONS_CLASSIFICATIONS."
        )
    if not isinstance(coverage_report.get("groups"), list):
        raise AwardPositionsClassificationsAnswerGapReportError("Coverage report must include groups[].")
    if not coverage_report["groups"]:
        raise AwardPositionsClassificationsAnswerGapReportError("Coverage report groups[] must not be empty.")
...
def _finding_for_group(group: dict) -> AwardPositionsClassificationsAnswerGapFinding:
    group_key = group["group_key"]
    status = group["coverage_status"]
    is_core = group_key in CORE_AWARD_POSITIONS_CLASSIFICATIONS_GROUPS

    if status == "STRONG":
        return AwardPositionsClassificationsAnswerGapFinding(
            group_key=group_key,
            ...
            rationale=(
                "Formal-corpus coverage is strong enough for the current Award Positions / Classifications answer "
                "standard."
            ),
        )
    if status == "WEAK":
        ...
        return AwardPositionsClassificationsAnswerGapFinding(
            ...
            rationale=(
                "Core Award Positions / Classifications support exists but may need clearer answer synthesis."
                if is_core
    ...
    return AwardPositionsClassificationsAnswerGapFinding(
        ...
        rationale=(
            "This is a core Award Positions / Classifications concept and missing evidence will materially weaken answers."
            if is_core
...
def _overall_status(findings: list[AwardPositionsClassificationsAnswerGapFinding]) -> str:
    core_findings = [finding for finding in findings if finding.group_key in CORE_AWARD_POSITIONS_CLASSIFICATIONS_GROUPS]
...
def _recommended_next_actions(
    findings: list[AwardPositionsClassificationsAnswerGapFinding],
    overall_status: str,
) -> list[str]:
    actions: list[str] = []
    if overall_status == "GOOD":
        actions.append(
            "Keep current Award Positions / Classifications retrieval terms and answer synthesis under benchmark watch."
        )
    if any(f.recommended_action == "ADD_FORMAL_SOURCE_EVIDENCE_LATER" for f in findings):
        actions.append(
            "Add formal source evidence later for missing Award Positions / Classifications groups before widening answer claims."
        )
    if any(f.recommended_action == "IMPROVE_RETRIEVAL_TERMS" for f in findings):
        actions.append(
            "Refine Award Positions / Classifications retrieval terms for weak supporting groups before adding new corpus."
        )
    if any(f.recommended_action == "IMPROVE_SYNTHESIS" for f in findings):
        actions.append("Tighten Award Positions / Classifications answer synthesis for weak core groups while keeping guardrails.")
    if overall_status == "INSUFFICIENT_CORPUS":
        actions.append("Treat Award Positions / Classifications answers as corpus-limited until core evidence groups are covered.")
    return list(dict.fromkeys(actions))


def build_award_positions_classifications_answer_gap_report(
    coverage_report: dict,
) -> AwardPositionsClassificationsAnswerGapReport:
    validate_award_positions_classifications_coverage_report(coverage_report)
    findings = [_finding_for_group(group) for group in coverage_report["groups"]]
    overall_status = _overall_status(findings)
    return AwardPositionsClassificationsAnswerGapReport(
        report_type="AWARD_POSITIONS_CLASSIFICATIONS_ANSWER_GAP_REPORT",
