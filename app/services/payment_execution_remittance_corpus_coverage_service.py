...
from app.services.domain_retrieval_plan_service import EvidenceGroup, PAYMENT_EXECUTION_REMITTANCE_PLAN
...
class PaymentExecutionRemittanceGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class PaymentExecutionRemittanceCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[PaymentExecutionRemittanceGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Payment Execution / Remittance claims.",
        ]
    return [
...
def scan_payment_execution_remittance_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> PaymentExecutionRemittanceCoverageReport:
    group_reports: list[PaymentExecutionRemittanceGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in PAYMENT_EXECUTION_REMITTANCE_PLAN.evidence_groups:
        ...
        group_reports.append(
            PaymentExecutionRemittanceGroupCoverage(
    ...
    return PaymentExecutionRemittanceCoverageReport(
        plan_id=PAYMENT_EXECUTION_REMITTANCE_PLAN.plan_id,
        domain=PAYMENT_EXECUTION_REMITTANCE_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(PAYMENT_EXECUTION_REMITTANCE_PLAN.evidence_groups),
