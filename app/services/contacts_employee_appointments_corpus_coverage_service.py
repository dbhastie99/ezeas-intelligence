...
from app.services.domain_retrieval_plan_service import CONTACTS_EMPLOYEE_APPOINTMENTS_PLAN, EvidenceGroup
from app.services.knowledge_retrieval_service import RetrievalResult, retrieve_relevant_chunks


@dataclass(frozen=True)
class ContactsEmployeeAppointmentsGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class ContactsEmployeeAppointmentsCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[ContactsEmployeeAppointmentsGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Contacts / Employee Appointments claims.",
        ]
    return [
...
def scan_contacts_employee_appointments_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> ContactsEmployeeAppointmentsCoverageReport:
    group_reports: list[ContactsEmployeeAppointmentsGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in CONTACTS_EMPLOYEE_APPOINTMENTS_PLAN.evidence_groups:
        ...
        group_reports.append(
            ContactsEmployeeAppointmentsGroupCoverage(
    ...
    return ContactsEmployeeAppointmentsCoverageReport(
        plan_id=CONTACTS_EMPLOYEE_APPOINTMENTS_PLAN.plan_id,
        domain=CONTACTS_EMPLOYEE_APPOINTMENTS_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(CONTACTS_EMPLOYEE_APPOINTMENTS_PLAN.evidence_groups),
