...
from app.services.domain_retrieval_plan_service import EvidenceGroup, FINALISATION_READINESS_PLAN
...
class FinalisationReadinessGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class FinalisationReadinessCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[FinalisationReadinessGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Finalisation Readiness claims.",
        ]
    return [
...
def scan_finalisation_readiness_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> FinalisationReadinessCoverageReport:
    group_reports: list[FinalisationReadinessGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in FINALISATION_READINESS_PLAN.evidence_groups:
        ...
        group_reports.append(
            FinalisationReadinessGroupCoverage(
    ...
    return FinalisationReadinessCoverageReport(
        plan_id=FINALISATION_READINESS_PLAN.plan_id,
        domain=FINALISATION_READINESS_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(FINALISATION_READINESS_PLAN.evidence_groups),
