...
from app.services.domain_retrieval_plan_service import EvidenceGroup, RETRO_REPLAY_PLAN
...
class RetroReplayGroupCoverage:
    group_key: str
    group_label: str
...
@dataclass(frozen=True)
class RetroReplayCoverageReport:
    plan_id: str
    domain: str
    ...
    groups: list[RetroReplayGroupCoverage]
...
def _diagnostic_notes(status: str, matched_term_count: int, matched_document_count: int) -> list[str]:
    ...
    if status == "WEAK":
        return [
            ...
            "Refine retrieval or synthesis before widening Retro / Replay claims.",
        ]
    return [
...
def scan_retro_replay_corpus_coverage(
    db: Session,
    top_k_per_group: int = 10,
    tenant_id: str | None = None,
) -> RetroReplayCoverageReport:
    group_reports: list[RetroReplayGroupCoverage] = []
    coverage_counts = {"STRONG": 0, "WEAK": 0, "MISSING": 0}

    for group in RETRO_REPLAY_PLAN.evidence_groups:
    ...
    return RetroReplayCoverageReport(
        plan_id=RETRO_REPLAY_PLAN.plan_id,
        domain=RETRO_REPLAY_PLAN.domain,
        generated_at_utc=datetime.now(UTC).isoformat(),
        total_evidence_groups=len(RETRO_REPLAY_PLAN.evidence_groups),
