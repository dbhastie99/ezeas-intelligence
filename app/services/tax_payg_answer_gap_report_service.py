...
class TaxPaygAnswerGapReportError(ValueError):
    pass


@dataclass(frozen=True)
class TaxPaygAnswerGapFinding:
    group_key: str
    coverage_status: str
    answer_impact: str
...
@dataclass(frozen=True)
class TaxPaygAnswerGapReport:
    report_type: str
    generated_at_utc: str
    overall_status: str
    group_findings: list[TaxPaygAnswerGapFinding]
...
def validate_tax_payg_coverage_report(coverage_report: dict) -> None:
    if not isinstance(coverage_report, dict):
        raise TaxPaygAnswerGapReportError("Coverage report must be a JSON object.")
    if coverage_report.get("plan_id") != "TAX_PAYG":
        raise TaxPaygAnswerGapReportError("Coverage report must have plan_id TAX_PAYG.")
    if not isinstance(coverage_report.get("groups"), list):
        raise TaxPaygAnswerGapReportError("Coverage report must include groups[].")
    if not coverage_report["groups"]:
        raise TaxPaygAnswerGapReportError("Coverage report groups[] must not be empty.")

    for index, group in enumerate(coverage_report["groups"]):
        if not isinstance(group, dict):
            raise TaxPaygAnswerGapReportError(f"Coverage group at index {index} must be an object.")
        group_key = group.get("group_key")
        coverage_status = group.get("coverage_status")
        if not group_key:
            raise TaxPaygAnswerGapReportError(f"Coverage group at index {index} is missing group_key.")
        if coverage_status not in VALID_COVERAGE_STATUSES:
            raise TaxPaygAnswerGapReportError(
...
def _finding_for_group(group: dict) -> TaxPaygAnswerGapFinding:
    group_key = group["group_key"]
    status = group["coverage_status"]
    is_core = group_key in CORE_TAX_PAYG_GROUPS

    if status == "STRONG":
        return TaxPaygAnswerGapFinding(
            group_key=group_key,
            coverage_status=status,
            answer_impact="LOW",
            diagnostic_summary=_diagnostic_summary(group),
            recommended_action="KEEP",
            rationale="Formal-corpus coverage is strong enough for the current Tax / PAYG answer standard.",
        )
    if status == "WEAK":
        ...
        return TaxPaygAnswerGapFinding(
            ...
            rationale=(
                "Core Tax / PAYG support exists but may need clearer answer synthesis."
                if is_core
    ...
    return TaxPaygAnswerGapFinding(
        ...
        rationale=(
            "This is a core Tax / PAYG concept and missing evidence will materially weaken answers."
            if is_core
...
def _overall_status(findings: list[TaxPaygAnswerGapFinding]) -> str:
    core_findings = [finding for finding in findings if finding.group_key in CORE_TAX_PAYG_GROUPS]
...
def _recommended_next_actions(findings: list[TaxPaygAnswerGapFinding], overall_status: str) -> list[str]:
    actions: list[str] = []
    if overall_status == "GOOD":
        actions.append("Keep current Tax / PAYG retrieval terms and answer synthesis under benchmark watch.")
    if any(f.recommended_action == "ADD_FORMAL_SOURCE_EVIDENCE_LATER" for f in findings):
        actions.append("Add formal source evidence later for missing Tax / PAYG groups before widening answer claims.")
    if any(f.recommended_action == "IMPROVE_RETRIEVAL_TERMS" for f in findings):
        actions.append("Refine Tax / PAYG retrieval terms for weak supporting groups before adding new corpus.")
    if any(f.recommended_action == "IMPROVE_SYNTHESIS" for f in findings):
        actions.append("Tighten Tax / PAYG answer synthesis for weak core groups while keeping status caveats.")
    if overall_status == "INSUFFICIENT_CORPUS":
        actions.append("Treat Tax / PAYG answers as corpus-limited until core evidence groups are covered.")
    return list(dict.fromkeys(actions))


def build_tax_payg_answer_gap_report(coverage_report: dict) -> TaxPaygAnswerGapReport:
    validate_tax_payg_coverage_report(coverage_report)
    findings = [_finding_for_group(group) for group in coverage_report["groups"]]
    overall_status = _overall_status(findings)
    return TaxPaygAnswerGapReport(
        report_type="TAX_PAYG_ANSWER_GAP_REPORT",
