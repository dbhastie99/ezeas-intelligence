...
from app.services.decision_story_answer_gap_report_service import build_decision_story_answer_gap_report
from scripts import build_decision_story_answer_gap_report as build_decision_story_gap_script
...
def _coverage_group(group_key: str, status: str) -> dict:
    ...
    return {
        ...
        "search_terms_used": ["Decision Story", group_key],
        "matched_chunk_count": matched,
        "matched_document_count": matched,
        "matched_sources": [],
        "representative_matched_terms": ["Decision Story"] if matched else [],
...
def test_decision_story_gap_report_good_when_groups_are_strong():
    report = build_decision_story_answer_gap_report(_coverage_report({})).to_dict()

    assert report["report_type"] == "DECISION_STORY_ANSWER_GAP_REPORT"
...
def test_decision_story_gap_report_maps_weak_group_actions():
    ...
    report = build_decision_story_answer_gap_report(coverage).to_dict()
...
def test_decision_story_gap_report_insufficient_corpus_when_core_groups_are_missing():
    coverage = _coverage_report(
        {
            "treatment_and_entitlement_selection": "MISSING",
            "decision_evidence_index": "MISSING",
        }
    )
    ...
    assert findings["decision_evidence_index"]["answer_impact"] == "HIGH"
    assert findings["decision_evidence_index"]["recommended_action"] == "ADD_FORMAL_SOURCE_EVIDENCE_LATER"
    assert findings["decision_evidence_index"]["rationale"]
    assert "Treat Decision Story answers as corpus-limited" in " ".join(report["recommended_next_actions"])


def test_decision_story_gap_report_group_action_mapping_for_supporting_missing_group():
...
def test_decision_story_gap_report_json_output_shape(tmp_path, monkeypatch, capsys):
    coverage_path = tmp_path / "coverage.json"
    coverage_path.write_text(json.dumps(_coverage_report({})), encoding="utf-8")
    monkeypatch.setattr(
        sys,
        "argv",
        ["build_decision_story_answer_gap_report.py", "--coverage-report", str(coverage_path), "--json"],
    )

    exit_code = build_decision_story_gap_script.main()
...
def test_decision_story_gap_report_writes_output_file(tmp_path, monkeypatch, capsys):
    coverage_path = tmp_path / "coverage.json"
    ...
    monkeypatch.setattr(
        ...
        "argv",
        [
            "build_decision_story_answer_gap_report.py",
            "--coverage-report",
            str(coverage_path),
            "--output",
            str(output_path),
    ...
    assert "Wrote Decision Story answer gap report" in captured.out
    assert report["overall_status"] == "NEEDS_REFINEMENT"


def test_decision_story_gap_report_human_output_is_diagnostic_only(tmp_path, monkeypatch, capsys):
    coverage_path = tmp_path / "coverage.json"
    coverage_path.write_text(json.dumps(_coverage_report({"decision_evidence_index": "WEAK"})), encoding="utf-8")
    monkeypatch.setattr(
        sys,
        "argv",
        ["build_decision_story_answer_gap_report.py", "--coverage-report", str(coverage_path)],
    )
...
def test_decision_story_gap_report_invalid_coverage_report_fails_clearly(tmp_path, monkeypatch, capsys):
    ...
    assert exit_code == 1
    assert "Decision Story answer gap report failed" in captured.out
    assert "plan_id DECISION_STORY" in captured.out
