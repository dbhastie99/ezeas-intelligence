...
HISTORICAL_RETRIEVAL_ELIGIBILITY_GATE = (
    HISTORICAL_KNOWLEDGE_ROOT / "HISTORICAL_RETRIEVAL_ELIGIBILITY_GATE.md"
)
HISTORICAL_RETRIEVAL_ELIGIBILITY_TEMPLATE = (
    HISTORICAL_KNOWLEDGE_ROOT / "HISTORICAL_RETRIEVAL_ELIGIBILITY_TEMPLATE.md"
)
HISTORICAL_RETRIEVAL_ELIGIBILITY_BLOCKER_MODEL = (
    HISTORICAL_KNOWLEDGE_ROOT / "HISTORICAL_RETRIEVAL_ELIGIBILITY_BLOCKER_MODEL.md"
)
HISTORICAL_RETRIEVAL_ANSWER_MODE_MAPPING = (
    HISTORICAL_KNOWLEDGE_ROOT / "HISTORICAL_RETRIEVAL_ANSWER_MODE_MAPPING.md"
)
...
HISTORICAL_RUNTIME_IMPLEMENTATION_DESIGN_PACK = (
    HISTORICAL_KNOWLEDGE_ROOT / "HISTORICAL_RUNTIME_IMPLEMENTATION_DESIGN_PACK.md"
)
HISTORICAL_RUNTIME_RETRIEVAL_GATE_DESIGN = (
    HISTORICAL_KNOWLEDGE_ROOT / "HISTORICAL_RUNTIME_RETRIEVAL_GATE_DESIGN.md"
)
HISTORICAL_RUNTIME_ANSWER_SYNTHESIS_GATE_DESIGN = (
    HISTORICAL_KNOWLEDGE_ROOT / "HISTORICAL_RUNTIME_ANSWER_SYNTHESIS_GATE_DESIGN.md"
)
HISTORICAL_RUNTIME_CITATION_REFUSAL_GATE_DESIGN = (
    HISTORICAL_KNOWLEDGE_ROOT / "HISTORICAL_RUNTIME_CITATION_REFUSAL_GATE_DESIGN.md"
)
HISTORICAL_RUNTIME_AUDIT_LOGGING_DESIGN = (
    HISTORICAL_KNOWLEDGE_ROOT / "HISTORICAL_RUNTIME_AUDIT_LOGGING_DESIGN.md"
)
...
HISTORICAL_ANSWER_SYNTHESIS_ENFORCEMENT_RESPONSE_CONTRACT = (
    HISTORICAL_KNOWLEDGE_ROOT
    / "HISTORICAL_ANSWER_SYNTHESIS_ENFORCEMENT_RESPONSE_CONTRACT.md"
)
HISTORICAL_ANSWER_SYNTHESIS_ENFORCEMENT_FIXTURE_CATALOG = (
    HISTORICAL_KNOWLEDGE_ROOT
    / "HISTORICAL_ANSWER_SYNTHESIS_ENFORCEMENT_FIXTURE_CATALOG.md"
)
HISTORICAL_ANSWER_SYNTHESIS_ENFORCEMENT_GUARDRAILS = (
    HISTORICAL_KNOWLEDGE_ROOT
    / "HISTORICAL_ANSWER_SYNTHESIS_ENFORCEMENT_GUARDRAILS.md"
)
...
def test_historical_retrieval_eligibility_docs_exist():
    assert HISTORICAL_RETRIEVAL_ELIGIBILITY_GATE.exists()
    assert HISTORICAL_RETRIEVAL_ELIGIBILITY_TEMPLATE.exists()
    assert HISTORICAL_RETRIEVAL_ELIGIBILITY_BLOCKER_MODEL.exists()
    assert HISTORICAL_RETRIEVAL_ANSWER_MODE_MAPPING.exists()
    assert HISTORICAL_RETRIEVAL_EXCLUSION_RULES.exists()
    assert HISTORICAL_RETRIEVAL_ELIGIBILITY_GATE_PROMPT.exists()


def test_historical_retrieval_eligibility_gate_required_model_and_dependencies():
    gate = _read(HISTORICAL_RETRIEVAL_ELIGIBILITY_GATE)

    for status in (
        "RETRIEVAL_ELIGIBILITY_NOT_REQUESTED",
        "RETRIEVAL_ELIGIBILITY_BLOCKED",
        "RETRIEVAL_ELIGIBILITY_DEFERRED",
        "RETRIEVAL_ELIGIBILITY_REJECTED",
        "RETRIEVAL_ELIGIBLE_HISTORICAL_CONTEXT_ONLY",
        "RETRIEVAL_ELIGIBLE_CURRENT_TRUTH",
        "RETRIEVAL_ELIGIBLE_WITH_CAVEAT",
        "RETRIEVAL_ELIGIBLE_BACKLOG_CONTEXT_ONLY",
        "RETRIEVAL_ELIGIBLE_DOCTRINE_CONTEXT_ONLY",
        "RETRIEVAL_EXCLUDED_NOT_ANSWERABLE",
        "RETRIEVAL_EXCLUDED_SUPERSEDED",
        "RETRIEVAL_EXCLUDED_CONFLICTED",
        "RETRIEVAL_REVOKED",
    ):
        assert status in gate
    ...
    assert "Answer-use permission is required before retrieval eligibility can be approved" in gate
    assert "Answer-use permission alone does not implement retrieval" in gate
    assert "Answer-use permission alone does not expose chat" in gate
    assert "Current-truth retrieval requires `CurrentTruthPermitted` Yes" in gate
    assert "Historical-context retrieval does not become current-truth retrieval" in gate


def test_historical_retrieval_eligibility_mapping_modes_exclusions_and_refusals():
    gate = _read(HISTORICAL_RETRIEVAL_ELIGIBILITY_GATE)
    mapping = _read(HISTORICAL_RETRIEVAL_ANSWER_MODE_MAPPING)
    exclusions = _read(HISTORICAL_RETRIEVAL_EXCLUSION_RULES)
    combined = "\n".join((gate, mapping, exclusions))
    ...
    for required_mapping in (
        ...
        assert required_mapping in gate
    ...
    for mode_rule in (
        ...
        assert mode_rule in gate

    for exclusion_rule in (
        ...
        assert exclusion_rule in combined

    assert "Conflicted evidence defaults to excluded from current-truth and settled-answer modes" in gate
    assert "Superseded evidence defaults to excluded from current-truth mode" in gate
    assert "If retrieval eligibility is absent, future Minerva answer logic must refuse or state insufficient governed evidence" in gate
    assert "If retrieval eligibility is blocked, future Minerva answer logic must refuse or state evidence is not retrieval-approved" in gate
    assert "This is a control document only and does not implement retrieval runtime or chat behaviour" in mapping


def test_historical_retrieval_eligibility_citation_blockers_and_boundaries_are_complete():
    gate = _read(HISTORICAL_RETRIEVAL_ELIGIBILITY_GATE)
    blocker_model = _read(HISTORICAL_RETRIEVAL_ELIGIBILITY_BLOCKER_MODEL)
    combined = "\n".join((gate, blocker_model))
    ...
    assert "Blocker resolution does not itself enable retrieval runtime" in blocker_model
    assert "Resolution does not expose chat" in blocker_model
    assert "Resolution does not call a live LLM" in blocker_model
    assert "Resolution does not make evidence answerable" in blocker_model


def test_historical_retrieval_eligibility_template_fields_and_defaults():
    template = _read(HISTORICAL_RETRIEVAL_ELIGIBILITY_TEMPLATE)
...
def test_historical_retrieval_eligibility_is_linked_from_prior_controls_and_index():
    index = _read(HISTORICAL_KNOWLEDGE_CONTROL_INDEX)
    answer_gate = _read(HISTORICAL_ANSWER_USE_PERMISSION_GATE)
    answer_template = _read(HISTORICAL_ANSWER_USE_PERMISSION_TEMPLATE)
    scope_rules = _read(HISTORICAL_ANSWER_USE_SCOPE_RULES)
    promotion_control = _read(HISTORICAL_CURRENT_TRUTH_PROMOTION_CONTROL)
    ingestion_control = _read(HISTORICAL_INGESTION_BACKFILL_DECISION_CONTROL)
    prompt = _read(HISTORICAL_RETRIEVAL_ELIGIBILITY_GATE_PROMPT)

    for linked_doc in (
        "HISTORICAL_RETRIEVAL_ELIGIBILITY_GATE.md",
        "HISTORICAL_RETRIEVAL_ELIGIBILITY_TEMPLATE.md",
        "HISTORICAL_RETRIEVAL_ELIGIBILITY_BLOCKER_MODEL.md",
        "HISTORICAL_RETRIEVAL_ANSWER_MODE_MAPPING.md",
        "HISTORICAL_RETRIEVAL_EXCLUSION_RULES.md",
    ):
        assert linked_doc in index
        assert linked_doc in prompt

    assert "Answer-use permission flows into retrieval eligibility and does not itself implement retrieval" in answer_gate
    assert "RetrievalEligibilityLink" in answer_template
    assert "Evidence scopes link to retrieval eligibility and answer modes" in scope_rules
    assert "Current-truth promotion requires answer-use permission and retrieval eligibility before future chat use" in promotion_control
    assert "Ingestion/backfill evidence must pass answer-use and retrieval eligibility gates before future answer use" in ingestion_control


def test_historical_retrieval_eligibility_slice_introduces_only_docs_tests_and_no_runtime_calls():
    changed = subprocess.run(
        ["git", "status", "--short"],
        capture_output=True,
        text=True,
    ...
    combined = "\n".join(
        ...
        for path in (
            ...
            HISTORICAL_CURRENT_TRUTH_PROMOTION_CONTROL,
            HISTORICAL_INGESTION_BACKFILL_DECISION_CONTROL,
            HISTORICAL_RETRIEVAL_ELIGIBILITY_GATE_PROMPT,
        )
    )
...
def test_historical_runtime_implementation_design_pack_docs_exist():
    assert HISTORICAL_RUNTIME_IMPLEMENTATION_DESIGN_PACK.exists()
    assert HISTORICAL_RUNTIME_RETRIEVAL_GATE_DESIGN.exists()
    assert HISTORICAL_RUNTIME_ANSWER_SYNTHESIS_GATE_DESIGN.exists()
    assert HISTORICAL_RUNTIME_CITATION_REFUSAL_GATE_DESIGN.exists()
    assert HISTORICAL_RUNTIME_AUDIT_LOGGING_DESIGN.exists()
    assert HISTORICAL_RUNTIME_IMPLEMENTATION_TEST_MATRIX_PLAN.exists()
    assert HISTORICAL_RUNTIME_IMPLEMENTATION_DESIGN_PACK_PROMPT.exists()


def test_historical_runtime_implementation_design_pack_core_controls():
    pack = _read(HISTORICAL_RUNTIME_IMPLEMENTATION_DESIGN_PACK)

    for status in (
        "RUNTIME_DESIGN_NOT_STARTED",
        "RUNTIME_DESIGN_DRAFTED",
        "RUNTIME_DESIGN_BLOCKED",
        "RUNTIME_DESIGN_DEFERRED",
        "RUNTIME_DESIGN_READY_FOR_TEST_MATRIX",
        "RUNTIME_DESIGN_READY_FOR_READ_ONLY_SKELETON_CANDIDATE",
        "RUNTIME_DESIGN_REJECTED",
        "RUNTIME_DESIGN_SUPERSEDED",
    ):
    ...
    for upstream_doc in (
        ...
        assert upstream_doc in pack

    assert (
        "request/query context -> retrieval gate -> answer-use gate -> answer-mode gate -> "
        "citation/provenance gate -> refusal gate -> audit/logging -> answer output or refusal"
    ) in pack
    assert "The architecture is design only and not implemented" in pack
...
def test_historical_runtime_implementation_design_pack_stop_conditions_and_non_goals():
    pack = _read(HISTORICAL_RUNTIME_IMPLEMENTATION_DESIGN_PACK)
    ...
    for non_goal in (
        ...
        assert non_goal in pack

    assert "Preferred next Minerva slice should be historical runtime implementation test matrix v0.1" in pack
    assert "Minerva has moved from chat pilot readiness control into runtime implementation design" in pack
    assert "Minerva remains pre-runtime and pre-chat" in pack
    assert "Estimated progress toward narrow safe internal chat pilot is about 85%" in pack


def test_historical_runtime_retrieval_gate_design_is_governed():
    design = _read(HISTORICAL_RUNTIME_RETRIEVAL_GATE_DESIGN)

    for required_text in (
        "No retrieval runtime is implemented",
        "request/query context",
        "answer-use permission status",
        "retrieval eligibility status",
        "current-truth promotion status",
        "Current-truth answer candidates require current-truth promotion",
        "Historical-context evidence may only be returned for approved historical-context answer modes",
        ...
        "RefusalRequired",
        "RefusalReason",
        "If retrieval is blocked, the gate must hand off refusal context",
        "does not implement runtime retrieval",
    ):
        assert required_text in design


def test_historical_runtime_answer_synthesis_gate_design_is_governed():
    design = _read(HISTORICAL_RUNTIME_ANSWER_SYNTHESIS_GATE_DESIGN)
    ...
    for required_text in (
        ...
        "Doctrine/context and hardening context answers must not be represented as runtime implementation evidence",
        "Current-truth-only answers require current-truth promotion",
        "DraftAnswerPayload",
        "RefusalPayload",
...
def test_historical_runtime_citation_refusal_gate_design_is_governed():
    design = _read(HISTORICAL_RUNTIME_CITATION_REFUSAL_GATE_DESIGN)
    ...
    for required_text in (
        ...
        "RetrievalEligibilityId",
        "AnswerModeId",
        "Non-refusal answers must have complete citation/provenance readiness",
        ...
        "superseded evidence",
        "not-answerable evidence",
        "The refusal must identify the missing or blocked gate where known",
...
def test_historical_runtime_audit_logging_design_is_governed():
    design = _read(HISTORICAL_RUNTIME_AUDIT_LOGGING_DESIGN)
    ...
    for required_text in (
        ...
        "answer-use gate decision",
        "answer-mode gate decision",
        "citation/provenance gate decision",
        "refusal gate decision",
        "must not mutate operational corpus content",
        "does not implement audit logging runtime",
    ):
        assert required_text in design


def test_historical_runtime_implementation_test_matrix_plan_groups_are_complete():
    ...
    for test_group in (
        ...
        assert test_group in plan


def test_historical_runtime_implementation_design_docs_are_linked_from_control_index():
    index = _read(HISTORICAL_KNOWLEDGE_CONTROL_INDEX)
    ...
    for status_text in (
        "Active runtime implementation design pack",
        "Active retrieval gate design",
        "Active answer synthesis gate design",
        "Active citation/refusal gate design",
        "Active audit/logging design",
...
def test_historical_runtime_implementation_test_matrix_slice_introduces_only_docs_tests_and_no_runtime_calls():
    ...
    for changed_file in changed_files:
        ...
        assert "answer_synthesis" not in normalized or (
...
def _approved_current_truth_retrieval_output() -> dict[str, object]:
    return {
        "RetrievalDecision": "ELIGIBLE_CURRENT_TRUTH_RETRIEVAL",
        "RetrievalMode": "READ_ONLY_METADATA_ONLY",
        "ExpectedAnswerMode": "CURRENT_TRUTH",
        "RefusalReason": None,
        "CitationRequired": True,
        "CaveatRequired": False,
...
def test_historical_answer_synthesis_enforcement_docs_and_service_exist():
    assert HISTORICAL_ANSWER_SYNTHESIS_ENFORCEMENT_SKELETON.exists()
    assert HISTORICAL_ANSWER_SYNTHESIS_ENFORCEMENT_RESPONSE_CONTRACT.exists()
    assert HISTORICAL_ANSWER_SYNTHESIS_ENFORCEMENT_FIXTURE_CATALOG.exists()
    assert HISTORICAL_ANSWER_SYNTHESIS_ENFORCEMENT_GUARDRAILS.exists()
    assert HISTORICAL_ANSWER_SYNTHESIS_ENFORCEMENT_SKELETON_SERVICE.exists()
    assert HISTORICAL_ANSWER_SYNTHESIS_ENFORCEMENT_SKELETON_PROMPT.exists()


def test_historical_answer_synthesis_enforcement_response_defaults_are_no_runtime():
    ...
    response = evaluate_historical_answer_synthesis_enforcement(
        _approved_current_truth_retrieval_output()
    )
    ...
    assert response["AnswerSynthesisSkeletonImplemented"] is True
    assert response["LiveLLMCalled"] is False
...
def test_historical_answer_synthesis_enforcement_allowed_answer_modes():
    from app.services.historical_answer_synthesis_enforcement_skeleton_service import (
        evaluate_historical_answer_synthesis_enforcement,
    )

    current_truth_response = evaluate_historical_answer_synthesis_enforcement(
        _approved_current_truth_retrieval_output()
    )
    assert current_truth_response["AnswerModeDecision"] == "CURRENT_TRUTH_ANSWER_ALLOWED"
    assert current_truth_response["AllowedAnswerMode"] == "CURRENT_TRUTH"
    assert current_truth_response["RefusalRequired"] is False

    historical_context = _approved_current_truth_retrieval_output()
    historical_context["RetrievalDecision"] = "ELIGIBLE_HISTORICAL_CONTEXT_RETRIEVAL"
    historical_context["ExpectedAnswerMode"] = "HISTORICAL_CONTEXT_ONLY"
    historical_context["EvidenceScope"] = "HISTORICAL_CONTEXT_ONLY"
    historical_context["AnswerMode"] = "HISTORICAL_CONTEXT"
    historical_context_response = evaluate_historical_answer_synthesis_enforcement(
        historical_context
    )
    assert historical_context_response["AnswerModeDecision"] == (
        "HISTORICAL_CONTEXT_ANSWER_ALLOWED"
    )
    assert historical_context_response["AllowedAnswerMode"] == "HISTORICAL_CONTEXT"
    assert historical_context_response["CaveatRequired"] is True

    caveated = _approved_current_truth_retrieval_output()
    caveated["RetrievalDecision"] = "ELIGIBLE_CAVEATED_RETRIEVAL"
    caveated["ExpectedAnswerMode"] = "CAVEATED_CURRENT_TRUTH"
    caveated["CaveatRequired"] = True
    caveated_response = evaluate_historical_answer_synthesis_enforcement(caveated)
    assert caveated_response["AnswerModeDecision"] == "CAVEATED_ANSWER_ALLOWED"
    assert caveated_response["AllowedAnswerMode"] == "CAVEATED"
    assert caveated_response["CaveatRequired"] is True


def test_historical_answer_synthesis_enforcement_refusals_and_blocks():
    from app.services.historical_answer_synthesis_enforcement_skeleton_service import (
        evaluate_historical_answer_synthesis_enforcement,
    )

    retrieval_refusal = _approved_current_truth_retrieval_output()
    retrieval_refusal["RetrievalDecision"] = "REFUSE_MISSING_RETRIEVAL_ELIGIBILITY"
    retrieval_refusal["RefusalReason"] = "RETRIEVAL_ELIGIBILITY_MISSING_OR_BLOCKED"
    retrieval_refusal_response = evaluate_historical_answer_synthesis_enforcement(
        retrieval_refusal
    )
    assert retrieval_refusal_response["AnswerModeDecision"] == (
        "REFUSE_RETRIEVAL_NOT_ELIGIBLE"
    )
    assert retrieval_refusal_response["RefusalRequired"] is True
    assert retrieval_refusal_response["RefusalReason"] == (
        "RETRIEVAL_ELIGIBILITY_MISSING_OR_BLOCKED"
    )

    missing_citation = _approved_current_truth_retrieval_output()
    missing_citation["SourceId"] = ""
    missing_citation_response = evaluate_historical_answer_synthesis_enforcement(
        missing_citation
    )
    assert missing_citation_response["AnswerModeDecision"] == "REFUSE_CITATION_REQUIRED"
    assert missing_citation_response["RefusalRequired"] is True

    conflicted = _approved_current_truth_retrieval_output()
    conflicted["ConflictStatus"] = "CONFLICTED"
    conflicted_response = evaluate_historical_answer_synthesis_enforcement(conflicted)
    assert conflicted_response["AnswerModeDecision"] == "REFUSE_CONFLICTED_EVIDENCE"
    assert conflicted_response["AllowedAnswerMode"] is None

    superseded = _approved_current_truth_retrieval_output()
    superseded["SupersessionStatus"] = "SUPERSEDED"
    superseded_response = evaluate_historical_answer_synthesis_enforcement(superseded)
    assert superseded_response["AnswerModeDecision"] == "REFUSE_SUPERSEDED_EVIDENCE"
    assert superseded_response["AllowedAnswerMode"] is None

    not_answerable = _approved_current_truth_retrieval_output()
    not_answerable["EvidenceScope"] = "NOT_ANSWERABLE"
    not_answerable_response = evaluate_historical_answer_synthesis_enforcement(
        not_answerable
    )
    assert not_answerable_response["AnswerModeDecision"] == "REFUSE_NOT_ANSWER_APPROVED"
    assert not_answerable_response["RefusalRequired"] is True

    runtime_required = _approved_current_truth_retrieval_output()
    runtime_required["LiveLLMRequired"] = True
    runtime_required_response = evaluate_historical_answer_synthesis_enforcement(
        runtime_required
    )
    assert runtime_required_response["AnswerModeDecision"] == (
        "BLOCKED_RUNTIME_NOT_IMPLEMENTED"
    )
    assert runtime_required_response["LiveLLMCalled"] is False
    assert runtime_required_response["FinalAnswerGenerated"] is False
    assert runtime_required_response["RetrievalRuntimeCalled"] is False
    assert runtime_required_response["DatabaseReadPerformed"] is False
    assert runtime_required_response["DatabaseWritePerformed"] is False


def test_historical_answer_synthesis_enforcement_decision_catalog_is_complete():
...
def test_historical_answer_synthesis_enforcement_docs_record_no_runtime_boundaries():
    combined = "\n".join(
        _read(path)
        for path in (
            HISTORICAL_ANSWER_SYNTHESIS_ENFORCEMENT_SKELETON,
    ...
    for boundary in (
        ...
        "No corpus mutation occurs",
        "No DB read/write occurs",
        "response is for gate enforcement only",
    ):
        assert boundary in combined


def test_historical_answer_synthesis_enforcement_links_from_runtime_controls():
    closeout = _read(HISTORICAL_READ_ONLY_GATED_RETRIEVAL_CONTRACT_CLOSEOUT)
    retrieval_contract = _read(HISTORICAL_READ_ONLY_GATED_RETRIEVAL_RESPONSE_CONTRACT)
    retrieval_catalog = _read(HISTORICAL_READ_ONLY_GATED_RETRIEVAL_DECISION_CATALOG)
    synthesis_design = _read(HISTORICAL_RUNTIME_ANSWER_SYNTHESIS_GATE_DESIGN)
    outcomes = _read(HISTORICAL_RUNTIME_IMPLEMENTATION_EXPECTED_OUTCOMES)
    assertions = _read(HISTORICAL_RUNTIME_IMPLEMENTATION_NO_RUNTIME_ASSERTIONS)
    index = _read(HISTORICAL_KNOWLEDGE_CONTROL_INDEX)
    ...
    for linked_doc in (
        ...
        assert linked_doc in closeout
        assert linked_doc in index

    assert "CURRENT_TRUTH_ANSWER_ALLOWED" in retrieval_contract
    assert "HISTORICAL_CONTEXT_ANSWER_ALLOWED" in retrieval_catalog
    assert "CAVEATED_ANSWER_ALLOWED" in synthesis_design
    assert "REFUSE_CITATION_REQUIRED" in outcomes
    assert "answer synthesis enforcement skeleton response is for gate enforcement only" in assertions


def test_historical_answer_synthesis_enforcement_slice_introduces_only_allowed_changes():
    changed = subprocess.run(
        ["git", "status", "--short"],
        capture_output=True,
        text=True,
...
def test_historical_chat_pilot_readiness_runtime_answer_citation_refusal_scope_and_entry_rules():
    ...
    assert "Chat pilot readiness flows into runtime implementation design, not chat exposure" in checklist
    assert "does not expose chat, implement retrieval runtime, implement answer synthesis runtime, implement citation rendering runtime" in checklist
...
def test_historical_chat_pilot_go_no_go_scope_blockers_and_entry_criteria_are_complete():
    ...
    assert "GO permits implementation design only unless later explicitly expanded" in go_no_go
    ...
    for criterion in (
        ...
        "no endpoint/UI yet",
        "no live LLM yet",
        "RuntimeImplementationDesignPackLink",
        "RuntimeImplementationTestMatrixPlanLink",
        "HISTORICAL_RUNTIME_IMPLEMENTATION_DESIGN_PACK.md",
        "HISTORICAL_RUNTIME_IMPLEMENTATION_TEST_MATRIX_PLAN.md",
    ):
...
def test_historical_runtime_gate_chain_blocker_template_and_chat_dependency_docs():
    ...
    assert "Runtime implementation design and runtime implementation test matrix are required before any pilot skeleton" in chain
    ...
    for dependency_name in (
        ...
        "HISTORICAL_RUNTIME_IMPLEMENTATION_DESIGN_PACK.md",
        "HISTORICAL_RUNTIME_IMPLEMENTATION_TEST_MATRIX_PLAN.md",
        "runtime implementation design pack",
        "runtime implementation test matrix plan",
        "answer-use gate",
...
def test_historical_runtime_gate_plan_slice_introduces_only_docs_tests_and_no_runtime_calls():
    ...
    combined = "\n".join(
        ...
        for path in (
            ...
            HISTORICAL_RUNTIME_IMPLEMENTATION_DESIGN_PACK_PROMPT,
        )
    )
