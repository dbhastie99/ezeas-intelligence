...
| Registered source type | Record `CHAT_OR_CONTINUANCE`, `PROMPT_FILE`, or `OTHER_REQUIRES_REVIEW` as assigned by the register. |
| Source tier | Record the starting reliability tier assigned by the register; usually Tier 3 unless a reviewed exception is documented. |
...
| Related control artefacts | Record linked prompts, logs, doctrine, review gates, inventories, or leave `unknown`. |
