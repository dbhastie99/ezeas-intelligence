# Payroll Output Corpus Coverage Baseline

This file records the Payroll Output corpus coverage baseline execution result for comparison control. It is diagnostic-only and not operational truth.
...
|---|---|
| `worker_level_output` | `WEAK` |
| Other Payroll Output evidence groups | 10 `STRONG` |
...
Generated artefact committed: no. The JSON output command created `.\artifacts\eval\payroll_output_corpus_coverage.json` locally for summarization, but the file is not a required committed artefact.
...
The only weak Payroll Output evidence group was `worker_level_output`. No group was reported as missing.
