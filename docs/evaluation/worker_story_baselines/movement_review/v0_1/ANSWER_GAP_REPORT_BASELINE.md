...
This file records the Movement Review answer gap report baseline execution result for comparison control. It is diagnostic-only and not operational truth.
...
Generated artefact committed: no. The JSON output command created `.\artifacts\eval\movement_review_answer_gap_report.json` locally for summarization, but the file is not a required committed artefact.
...
Each reported Movement Review evidence group was `STRONG` -> `KEEP`.
...
The answer gap report recommended keeping current Movement Review retrieval terms and answer synthesis under benchmark watch.
