# Core Payroll Explanation Baseline Batch v0.1

Date: 2026-05-13

This batch inspected four selected `BASELINE_REQUIRED` domains and stopped at DB readiness because the shared read-only readiness check returned `DATABASE_CONNECTION_FAILED`.
...
| --- | --- | --- | --- | --- | --- | --- |
| Finalisation Readiness | `DATABASE_CONNECTION_FAILED` | blocked pack only | not run | not run | not run | `BASELINE_REQUIRED` |
| Payroll Output | `DATABASE_CONNECTION_FAILED` | blocked pack only | not run | not run | not run | `BASELINE_REQUIRED` |
| RateSource / Rate Story | `DATABASE_CONNECTION_FAILED` | blocked pack only | not run | not run | not run | `BASELINE_REQUIRED` |
| Decision Story | `DATABASE_CONNECTION_FAILED` | blocked pack only | not run | not run | not run | `BASELINE_REQUIRED` |
