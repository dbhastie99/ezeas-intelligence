# Annual Leave / Leave Management Corpus Coverage Baseline

This file records the Annual Leave / Leave Management corpus coverage baseline execution result for comparison control. It is diagnostic-only and not operational truth.
...
Plan id: `ANNUAL_LEAVE_MANAGEMENT`
...
Generated artefact committed: no. The JSON output command created `.\artifacts\eval\annual_leave_corpus_coverage.json` locally for summarization, but the file is not a required committed artefact.
...
All 7 Annual Leave / Leave Management evidence groups were reported as `STRONG`.
