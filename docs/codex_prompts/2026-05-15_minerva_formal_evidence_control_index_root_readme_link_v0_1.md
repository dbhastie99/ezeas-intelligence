...
Update `README.md` so future Minerva and Codex work can discover `docs/evaluation/source_evidence_drafts/FORMAL_EVIDENCE_CONTROL_INDEX.md` as the master starting point/control index for the formal evidence control model.

The root README update must be concise. It must not duplicate the full control model.

## Required Root README Navigation

The root README navigation text must state that `docs/evaluation/source_evidence_drafts/FORMAL_EVIDENCE_CONTROL_INDEX.md` is the master starting point/control index for:
...
Update `tests/test_domain_baseline_capture_batch.py` or an appropriate existing README/navigation test to assert that `README.md`:
...
- names the required control areas without duplicating the full model
...
Suggested commit message: `minerva-formal-evidence-control-index-root-readme-link-v01`
