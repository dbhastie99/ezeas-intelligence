...
Create `docs/evaluation/source_evidence_drafts/FORMAL_EVIDENCE_RECAPTURE_PLANNING_RUNBOOK.md` as the controlled planning process for recapturing a baseline after a future governed corpus-ingestion slice has completed.
...
The runbook must state that current Tax / PAYG and Imports / Actuals records are `NOT_REVIEWED`, so recapture is currently not permitted for either domain.

The runbook must define preconditions for future recapture planning:
...
The runbook must state that recapture planning alone:
...
- does not permit Minerva to answer as if recapture has happened

The runbook must state that actual recapture must be a separate explicit slice after governed ingestion evidence exists.
...
- the runbook defines all recapture planning preconditions
- the runbook preserves current blocked states for Tax / PAYG and Imports / Actuals
- the runbook states recapture planning alone does not mutate corpus, run benchmark, run corpus coverage, run answer-gap reporting, promote a baseline, change runtime, or change ledger counts
- the runbook does not state that recapture has occurred, benchmark has passed, corpus coverage has passed, answer gap is GOOD, ledger has been promoted, Tax / PAYG is `BASELINE_ALREADY_EXISTS`, or Imports / Actuals is `BASELINE_ALREADY_EXISTS`
...
Suggested commit message: `minerva-formal-evidence-recapture-planning-runbook-v01`
