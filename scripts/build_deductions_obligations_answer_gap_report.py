...
def print_report(report: dict) -> None:
    print(f"Deductions / Obligations answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build a Deductions / Obligations answer-quality gap report from coverage JSON.")
    ...
    from app.services.deductions_obligations_answer_gap_report_service import (
        DeductionsObligationsAnswerGapReportError,
        build_deductions_obligations_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_deductions_obligations_answer_gap_report(coverage_report).to_dict()
    except (ValueError, DeductionsObligationsAnswerGapReportError) as exc:
        print(f"Deductions / Obligations answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Deductions / Obligations answer gap report to {output_path}")
