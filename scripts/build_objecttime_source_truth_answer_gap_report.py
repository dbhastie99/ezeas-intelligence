...
def print_report(report: dict) -> None:
    print(f"ObjectTime / Source Truth answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build an ObjectTime / Source Truth answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.objecttime_source_truth_answer_gap_report_service import (
        ObjectTimeSourceTruthAnswerGapReportError,
        build_objecttime_source_truth_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_objecttime_source_truth_answer_gap_report(coverage_report).to_dict()
    except (ValueError, ObjectTimeSourceTruthAnswerGapReportError) as exc:
        print(f"ObjectTime / Source Truth answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote ObjectTime / Source Truth answer gap report to {output_path}")
