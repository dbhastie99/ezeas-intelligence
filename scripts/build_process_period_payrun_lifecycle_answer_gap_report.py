...
def print_report(report: dict) -> None:
    print(f"Process Periods / PayRun Lifecycle answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a Process Periods / PayRun Lifecycle answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.process_period_payrun_lifecycle_answer_gap_report_service import (
        ProcessPeriodPayRunLifecycleAnswerGapReportError,
        build_process_period_payrun_lifecycle_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_process_period_payrun_lifecycle_answer_gap_report(coverage_report).to_dict()
    except (ValueError, ProcessPeriodPayRunLifecycleAnswerGapReportError) as exc:
        print(f"Process Periods / PayRun Lifecycle answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Process Periods / PayRun Lifecycle answer gap report to {output_path}")
