...
def print_report(report: dict) -> None:
    print(f"Payroll Bases & Totals answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a Payroll Bases & Totals answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.payroll_bases_answer_gap_report_service import (
        PayrollBasesAnswerGapReportError,
        build_payroll_bases_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_payroll_bases_answer_gap_report(coverage_report).to_dict()
    except (ValueError, PayrollBasesAnswerGapReportError) as exc:
        print(f"Payroll Bases & Totals answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Payroll Bases & Totals answer gap report to {output_path}")
