...
def print_report(report: dict) -> None:
    print(f"Decision Story answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build a Decision Story answer-quality gap report from coverage JSON.")
    parser.add_argument("--coverage-report", required=True)
    parser.add_argument("--output")
    parser.add_argument("--json", action="store_true", help="Print the report as JSON.")
    args = parser.parse_args()

    from app.services.decision_story_answer_gap_report_service import (
        DecisionStoryAnswerGapReportError,
        build_decision_story_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_decision_story_answer_gap_report(coverage_report).to_dict()
    except (ValueError, DecisionStoryAnswerGapReportError) as exc:
        print(f"Decision Story answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Decision Story answer gap report to {output_path}")
