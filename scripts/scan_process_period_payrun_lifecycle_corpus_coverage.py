...
def print_report(report: dict) -> None:
    print(f"Process Periods / PayRun Lifecycle corpus coverage: {report['plan_id']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Scan formal corpus coverage for Process Periods / PayRun Lifecycle evidence groups."
    )
    ...
    try:
        with SessionLocal() as db:
            report = scan_process_period_payrun_lifecycle_corpus_coverage(
                db=db, top_k_per_group=args.top_k_per_group
            ).to_dict()
    except SQLAlchemyError as exc:
        print(f"Process Periods / PayRun Lifecycle corpus coverage scan failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Process Periods / PayRun Lifecycle corpus coverage report to {output_path}")
