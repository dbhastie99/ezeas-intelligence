...
def print_report(report: dict) -> None:
    print(f"Payment Execution / Remittance corpus coverage: {report['plan_id']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Scan formal corpus coverage for Payment Execution / Remittance evidence groups."
    )
    ...
    try:
        with SessionLocal() as db:
            report = scan_payment_execution_remittance_corpus_coverage(
                db=db,
                top_k_per_group=args.top_k_per_group,
            ).to_dict()
    except SQLAlchemyError as exc:
        print(f"Payment Execution / Remittance corpus coverage scan failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Payment Execution / Remittance corpus coverage report to {output_path}")
