...
def print_report(report: dict) -> None:
    print(f"PayRun Admin Queue corpus coverage: {report['plan_id']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scan formal corpus coverage for PayRun Admin Queue evidence groups.")
    ...
    try:
        with SessionLocal() as db:
            report = scan_payrun_admin_queue_corpus_coverage(db=db, top_k_per_group=args.top_k_per_group).to_dict()
    except SQLAlchemyError as exc:
        print(f"PayRun Admin Queue corpus coverage scan failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote PayRun Admin Queue corpus coverage report to {output_path}")
