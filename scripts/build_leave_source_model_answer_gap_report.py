...
def print_report(report: dict) -> None:
    print(f"Leave Source Model answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a Leave Source Model answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.leave_source_model_answer_gap_report_service import (
        LeaveSourceModelAnswerGapReportError,
        build_leave_source_model_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_leave_source_model_answer_gap_report(coverage_report).to_dict()
    except (ValueError, LeaveSourceModelAnswerGapReportError) as exc:
        print(f"Leave Source Model answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Leave Source Model answer gap report to {output_path}")
