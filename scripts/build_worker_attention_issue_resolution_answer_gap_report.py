...
def print_report(report: dict) -> None:
    print(f"Worker Attention / Issue Resolution answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a Worker Attention / Issue Resolution answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.worker_attention_issue_resolution_answer_gap_report_service import (
        WorkerAttentionIssueResolutionAnswerGapReportError,
        build_worker_attention_issue_resolution_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_worker_attention_issue_resolution_answer_gap_report(coverage_report).to_dict()
    except (ValueError, WorkerAttentionIssueResolutionAnswerGapReportError) as exc:
        print(f"Worker Attention / Issue Resolution answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Worker Attention / Issue Resolution answer gap report to {output_path}")
