...
def print_report(report: dict) -> None:
    print(f"Comparison / Remediation answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a Comparison / Remediation answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.comparison_remediation_answer_gap_report_service import (
        ComparisonRemediationAnswerGapReportError,
        build_comparison_remediation_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_comparison_remediation_answer_gap_report(coverage_report).to_dict()
    except (ValueError, ComparisonRemediationAnswerGapReportError) as exc:
        print(f"Comparison / Remediation answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Comparison / Remediation answer gap report to {output_path}")
