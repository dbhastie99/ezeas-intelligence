...
def print_report(report: dict) -> None:
    print(f"On-costs / Employer Liabilities answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build an On-costs / Employer Liabilities answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.oncosts_employer_liabilities_answer_gap_report_service import (
        OncostsEmployerLiabilitiesAnswerGapReportError,
        build_oncosts_employer_liabilities_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_oncosts_employer_liabilities_answer_gap_report(coverage_report).to_dict()
    except (ValueError, OncostsEmployerLiabilitiesAnswerGapReportError) as exc:
        print(f"On-costs / Employer Liabilities answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote On-costs / Employer Liabilities answer gap report to {output_path}")
