...
def print_report(report: dict) -> None:
    print(f"Payment Execution / Remittance answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a Payment Execution / Remittance answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.payment_execution_remittance_answer_gap_report_service import (
        PaymentExecutionRemittanceAnswerGapReportError,
        build_payment_execution_remittance_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_payment_execution_remittance_answer_gap_report(coverage_report).to_dict()
    except (ValueError, PaymentExecutionRemittanceAnswerGapReportError) as exc:
        print(f"Payment Execution / Remittance answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Payment Execution / Remittance answer gap report to {output_path}")
