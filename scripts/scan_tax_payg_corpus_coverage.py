...
def print_report(report: dict) -> None:
    print(f"Tax / PAYG corpus coverage: {report['plan_id']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scan formal corpus coverage for Tax / PAYG evidence groups.")
    ...
    try:
        with SessionLocal() as db:
            report = scan_tax_payg_corpus_coverage(db=db, top_k_per_group=args.top_k_per_group).to_dict()
    except SQLAlchemyError as exc:
        print(f"Tax / PAYG corpus coverage scan failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Tax / PAYG corpus coverage report to {output_path}")
