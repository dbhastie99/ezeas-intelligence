...
def print_report(report: dict) -> None:
    print(f"Contacts / Employee Appointments answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a Contacts / Employee Appointments answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.contacts_employee_appointments_answer_gap_report_service import (
        ContactsEmployeeAppointmentsAnswerGapReportError,
        build_contacts_employee_appointments_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_contacts_employee_appointments_answer_gap_report(coverage_report).to_dict()
    except (ValueError, ContactsEmployeeAppointmentsAnswerGapReportError) as exc:
        print(f"Contacts / Employee Appointments answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Contacts / Employee Appointments answer gap report to {output_path}")
