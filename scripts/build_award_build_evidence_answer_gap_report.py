...
def print_report(report: dict) -> None:
    print(f"Award Build / Award Evidence answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build an Award Build / Award Evidence answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.award_build_evidence_answer_gap_report_service import (
        AwardBuildEvidenceAnswerGapReportError,
        build_award_build_evidence_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_award_build_evidence_answer_gap_report(coverage_report).to_dict()
    except (ValueError, AwardBuildEvidenceAnswerGapReportError) as exc:
        print(f"Award Build / Award Evidence answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Award Build / Award Evidence answer gap report to {output_path}")
