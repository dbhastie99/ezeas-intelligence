...
def print_report(report: dict) -> None:
    print(f"RateSource / Rate Story answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a RateSource / Rate Story answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.rate_source_rate_story_answer_gap_report_service import (
        RateSourceRateStoryAnswerGapReportError,
        build_rate_source_rate_story_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_rate_source_rate_story_answer_gap_report(coverage_report).to_dict()
    except (ValueError, RateSourceRateStoryAnswerGapReportError) as exc:
        print(f"RateSource / Rate Story answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote RateSource / Rate Story answer gap report to {output_path}")
