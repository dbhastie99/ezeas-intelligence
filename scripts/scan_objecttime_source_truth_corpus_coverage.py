...
def print_report(report: dict) -> None:
    print(f"ObjectTime / Source Truth corpus coverage: {report['plan_id']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Scan formal corpus coverage for ObjectTime / Source Truth evidence groups."
    )
    ...
    from app.services.objecttime_source_truth_corpus_coverage_service import (
        scan_objecttime_source_truth_corpus_coverage,
    )

    try:
        with SessionLocal() as db:
            report = scan_objecttime_source_truth_corpus_coverage(
                db=db,
                top_k_per_group=args.top_k_per_group,
            ).to_dict()
    except SQLAlchemyError as exc:
        print(f"ObjectTime / Source Truth corpus coverage scan failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote ObjectTime / Source Truth corpus coverage report to {output_path}")
