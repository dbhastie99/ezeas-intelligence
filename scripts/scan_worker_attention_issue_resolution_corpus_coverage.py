...
def print_report(report: dict) -> None:
    print(f"Worker Attention / Issue Resolution corpus coverage: {report['plan_id']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Scan formal corpus coverage for Worker Attention / Issue Resolution groups."
    )
    ...
    try:
        with SessionLocal() as db:
            report = scan_worker_attention_issue_resolution_corpus_coverage(
                db=db, top_k_per_group=args.top_k_per_group
            ).to_dict()
    except SQLAlchemyError as exc:
        print(f"Worker Attention / Issue Resolution corpus coverage scan failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Worker Attention / Issue Resolution corpus coverage report to {output_path}")
