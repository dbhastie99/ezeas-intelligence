...
def print_report(report: dict) -> None:
    print(f"Award Positions / Classifications answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build an Award Positions / Classifications answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.award_positions_classifications_answer_gap_report_service import (
        AwardPositionsClassificationsAnswerGapReportError,
        build_award_positions_classifications_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_award_positions_classifications_answer_gap_report(coverage_report).to_dict()
    except (ValueError, AwardPositionsClassificationsAnswerGapReportError) as exc:
        print(f"Award Positions / Classifications answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Award Positions / Classifications answer gap report to {output_path}")
