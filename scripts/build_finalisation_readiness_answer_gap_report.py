...
def print_report(report: dict) -> None:
    print(f"Finalisation Readiness answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a Finalisation Readiness answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.finalisation_readiness_answer_gap_report_service import (
        FinalisationReadinessAnswerGapReportError,
        build_finalisation_readiness_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_finalisation_readiness_answer_gap_report(coverage_report).to_dict()
    except (ValueError, FinalisationReadinessAnswerGapReportError) as exc:
        print(f"Finalisation Readiness answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Finalisation Readiness answer gap report to {output_path}")
