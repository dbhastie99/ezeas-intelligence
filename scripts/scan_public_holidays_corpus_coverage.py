...
def print_report(report: dict) -> None:
    print(f"Public Holidays corpus coverage: {report['plan_id']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scan formal corpus coverage for Public Holidays groups.")
    ...
    try:
        with SessionLocal() as db:
            report = scan_public_holidays_corpus_coverage(db=db, top_k_per_group=args.top_k_per_group).to_dict()
    except SQLAlchemyError as exc:
        print(f"Public Holidays corpus coverage scan failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Public Holidays corpus coverage report to {output_path}")
