...
def print_report(report: dict) -> None:
    print(f"Imports / Actuals answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build an Imports / Actuals answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.imports_actuals_answer_gap_report_service import (
        ImportsActualsAnswerGapReportError,
        build_imports_actuals_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_imports_actuals_answer_gap_report(coverage_report).to_dict()
    except (ValueError, ImportsActualsAnswerGapReportError) as exc:
        print(f"Imports / Actuals answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Imports / Actuals answer gap report to {output_path}")
