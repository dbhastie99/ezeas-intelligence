...
def print_report(report: dict) -> None:
    print(f"Retro / Replay answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build a Retro / Replay answer-quality gap report from coverage JSON.")
    parser.add_argument("--coverage-report", required=True)
    parser.add_argument("--output")
    parser.add_argument("--json", action="store_true", help="Print the report as JSON.")
    args = parser.parse_args()

    from app.services.retro_replay_answer_gap_report_service import (
        RetroReplayAnswerGapReportError,
        build_retro_replay_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_retro_replay_answer_gap_report(coverage_report).to_dict()
    except (ValueError, RetroReplayAnswerGapReportError) as exc:
        print(f"Retro / Replay answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Retro / Replay answer gap report to {output_path}")
