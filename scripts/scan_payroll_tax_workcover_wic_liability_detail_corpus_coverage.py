...
def print_report(report: dict) -> None:
    print(f"Payroll Tax / WorkCover / WIC Liability Detail corpus coverage: {report['plan_id']}")
    ...
    for group in report["groups"]:
        ...
        sources = ", ".join(source["title"] or source["original_file_name"] for source in group["matched_sources"][:3])
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Scan formal corpus coverage for Payroll Tax / WorkCover / WIC Liability Detail groups."
    )
    ...
    try:
        with SessionLocal() as db:
            report = scan_payroll_tax_workcover_wic_liability_detail_corpus_coverage(
                db=db,
                top_k_per_group=args.top_k_per_group,
            ).to_dict()
    except SQLAlchemyError as exc:
        print(f"Payroll Tax / WorkCover / WIC Liability Detail corpus coverage scan failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Payroll Tax / WorkCover / WIC Liability Detail corpus coverage report to {output_path}")
