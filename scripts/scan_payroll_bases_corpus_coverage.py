...
def print_report(report: dict) -> None:
    print(f"Payroll Bases & Totals corpus coverage: {report['plan_id']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scan formal corpus coverage for Payroll Bases & Totals evidence groups.")
    ...
    try:
        with SessionLocal() as db:
            report = scan_payroll_bases_corpus_coverage(db=db, top_k_per_group=args.top_k_per_group).to_dict()
    except SQLAlchemyError as exc:
        print(f"Payroll Bases & Totals corpus coverage scan failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Payroll Bases & Totals corpus coverage report to {output_path}")
