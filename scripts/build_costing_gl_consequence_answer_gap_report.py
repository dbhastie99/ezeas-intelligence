...
def print_report(report: dict) -> None:
    print(f"Costing / GL Consequence Evidence answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a Costing / GL Consequence Evidence answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.costing_gl_consequence_answer_gap_report_service import (
        CostingGlConsequenceAnswerGapReportError,
        build_costing_gl_consequence_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_costing_gl_consequence_answer_gap_report(coverage_report).to_dict()
    except (ValueError, CostingGlConsequenceAnswerGapReportError) as exc:
        print(f"Costing / GL Consequence Evidence answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Costing / GL Consequence Evidence answer gap report to {output_path}")
