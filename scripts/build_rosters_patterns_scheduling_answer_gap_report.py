...
def print_report(report: dict) -> None:
    print(f"Rosters / Patterns / Scheduling answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a Rosters / Patterns / Scheduling answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.rosters_patterns_scheduling_answer_gap_report_service import (
        RostersPatternsSchedulingAnswerGapReportError,
        build_rosters_patterns_scheduling_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_rosters_patterns_scheduling_answer_gap_report(coverage_report).to_dict()
    except (ValueError, RostersPatternsSchedulingAnswerGapReportError) as exc:
        print(f"Rosters / Patterns / Scheduling answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Rosters / Patterns / Scheduling answer gap report to {output_path}")
