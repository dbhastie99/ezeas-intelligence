...
def print_report(report: dict) -> None:
    print(f"Costing / GL Consequence Evidence corpus coverage: {report['plan_id']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Scan formal corpus coverage for Costing / GL Consequence Evidence groups."
    )
    ...
    try:
        with SessionLocal() as db:
            report = scan_costing_gl_consequence_corpus_coverage(
                db=db, top_k_per_group=args.top_k_per_group
            ).to_dict()
    except SQLAlchemyError as exc:
        print(f"Costing / GL Consequence Evidence corpus coverage scan failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Costing / GL Consequence Evidence corpus coverage report to {output_path}")
