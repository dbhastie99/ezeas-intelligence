...
def print_report(report: dict) -> None:
    print(f"Annual Leave / Leave Management corpus coverage: {report['plan_id']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Scan formal corpus coverage for Annual Leave / Leave Management evidence groups."
    )
    ...
    try:
        with SessionLocal() as db:
            report = scan_annual_leave_corpus_coverage(db=db, top_k_per_group=args.top_k_per_group).to_dict()
    except SQLAlchemyError as exc:
        print(f"Annual Leave / Leave Management corpus coverage scan failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Annual Leave / Leave Management corpus coverage report to {output_path}")
