...
def print_report(report: dict) -> None:
    print(f"Leave Accrual / Processing answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a Leave Accrual / Processing answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.leave_accrual_processing_answer_gap_report_service import (
        LeaveAccrualProcessingAnswerGapReportError,
        build_leave_accrual_processing_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_leave_accrual_processing_answer_gap_report(coverage_report).to_dict()
    except (ValueError, LeaveAccrualProcessingAnswerGapReportError) as exc:
        print(f"Leave Accrual / Processing answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Leave Accrual / Processing answer gap report to {output_path}")
