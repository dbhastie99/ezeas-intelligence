...
def print_report(report: dict) -> None:
    print(f"Annual Leave / Leave Management answer gap report: {report['overall_status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build an Annual Leave / Leave Management answer-quality gap report from coverage JSON."
    )
    ...
    from app.services.annual_leave_answer_gap_report_service import (
        AnnualLeaveAnswerGapReportError,
        build_annual_leave_answer_gap_report,
    )

    try:
        coverage_report = _load_coverage_report(Path(args.coverage_report))
        report = build_annual_leave_answer_gap_report(coverage_report).to_dict()
    except (ValueError, AnnualLeaveAnswerGapReportError) as exc:
        print(f"Annual Leave / Leave Management answer gap report failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Annual Leave / Leave Management answer gap report to {output_path}")
