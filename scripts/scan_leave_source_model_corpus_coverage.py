...
def print_report(report: dict) -> None:
    print(f"Leave Source Model corpus coverage: {report['plan_id']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Scan formal corpus coverage for Leave Source Model evidence groups."
    )
    ...
    try:
        with SessionLocal() as db:
            report = scan_leave_source_model_corpus_coverage(
                db=db,
                top_k_per_group=args.top_k_per_group,
            ).to_dict()
    except SQLAlchemyError as exc:
        print(f"Leave Source Model corpus coverage scan failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Leave Source Model corpus coverage report to {output_path}")
