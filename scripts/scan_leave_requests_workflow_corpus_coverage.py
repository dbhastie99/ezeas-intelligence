...
def print_report(report: dict) -> None:
    print(f"Leave Requests / Leave Workflow corpus coverage: {report['plan_id']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scan formal corpus coverage for Leave Requests / Leave Workflow groups.")
    ...
    try:
        with SessionLocal() as db:
            report = scan_leave_requests_workflow_corpus_coverage(db=db, top_k_per_group=args.top_k_per_group).to_dict()
    except SQLAlchemyError as exc:
        print(f"Leave Requests / Leave Workflow corpus coverage scan failed: {exc}")
        return 1
    ...
    if args.output:
        ...
        if not args.json:
            print(f"Wrote Leave Requests / Leave Workflow corpus coverage report to {output_path}")
