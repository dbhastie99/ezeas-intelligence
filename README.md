...
Worker Story baseline capture policy and the first checked-in baseline pack are documented under [docs/evaluation/worker_story_baselines/](docs/evaluation/worker_story_baselines/), with the DB readiness check documented in [WORKER_STORY_BASELINE_DB_READINESS.md](docs/evaluation/worker_story_baselines/WORKER_STORY_BASELINE_DB_READINESS.md). The v0.3 captured baseline records READY database readiness, benchmark result 5 total / 4 passed / 1 failed, corpus coverage 10 STRONG / 0 WEAK / 0 MISSING and answer gap status GOOD with all groups KEEP. These artefacts are comparison controls only; they do not mutate corpus, change routing, change answer generation, call a live LLM, ingest operational JSON, connect Code Evidence, or prove runtime platform truth.
